...
function pwaInstallState(){
  ...
  if(!pwaSecureContext())return {state:'https-required',title:'HTTPS required',message:'A trusted HTTPS connection is required before this device can install the VeloraOS PWA.'};
...
async function installModel(id){
  var model=allModels(true).find(function(item){return item.id===id;});if(!model)return;var availability=model.availability||{};
  if(availability.hard_block){var msg=(availability.reasons||['There is not enough free disk space.']).join(' ');pushNotification({id:'model-space-'+id+'-'+Date.now(),type:'warning',title:'Model download blocked',message:model.name+': '+msg,action:'models',actionLabel:'Open Models',priority:'warning'});render();return;}
  S.modelTasks[id]={status:'queued',progress:0,output:'Preparing download…'};render();
  try{
    var start=await API.post('/api/models/'+encodeURIComponent(id)+'/install',{force:true,riskAccepted:true});S.modelTasks[id].task_id=start.task_id;
    for(var i=0;i<720;i++){
      await new Promise(function(resolve){setTimeout(resolve,1000);});var task=await API.get('/api/tasks/'+encodeURIComponent(start.task_id));S.modelTasks[id]=task;render();var state=String(task.status||'').toLowerCase();
      if(['complete','completed','done','success'].indexOf(state)>=0){pushNotification({id:'model-installed-'+id+'-'+Date.now(),type:'model',title:model.name+' installed',message:'The model is ready to use in Chat.',action:'chat',actionLabel:'Start chatting',priority:'success'});var refreshed=await API.get('/api/models');S.models=refreshed.models||refreshed||[];delete S.modelTasks[id];render();return;}
      if(state==='cancelled'){pushNotification({id:'model-cancelled-'+id+'-'+Date.now(),type:'activity',title:model.name+' download cancelled',message:'You can retry the download at any time from Models.',action:'models',actionLabel:'Open Models',priority:'activity'});render();return;}
      if(['error','failed'].indexOf(state)>=0)throw new Error(task.error||'Install failed');
    }
    throw new Error('The model download did not finish in time.');
  }catch(error){S.modelTasks[id]=Object.assign({},S.modelTasks[id]||{},{status:'error',error:humanError(error)});pushNotification({id:'model-failed-'+id+'-'+Date.now(),type:'warning',title:model.name+' download failed',message:humanError(error),action:'models',actionLabel:'Retry',priority:'warning'});render();}
}
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.8')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
function systemInfoPage(){
  return '<div class="section-head"><div><p class="eyebrow">System administration</p><h2>System Info</h2><p class="lead">Diagnostics, hardware health, backups and recovery tools in one place.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div>'+systemHealthOverview()+
...
function pwaSettingsCard(){
  ...
  else if(info.state==='https-required')button='<button class="btn ghost" disabled>HTTPS required</button>';
  else button='<button class="btn ghost" disabled>Installed</button>';
  return '<div class="setting-card pwa-setting-card"><p class="eyebrow">Mobile app</p><h3>Install VeloraOS</h3><p class="muted">'+escapeHtml(info.message)+'</p><div class="pwa-install-actions">'+button+'</div><p class="small muted">Only the app shell and static assets are cached. Authenticated APIs, chats, settings, licence information and model data stay network-only on your VeloraOS appliance.</p></div>';
}
...
window.VELORAOS_RELEASE="1.10.8";
