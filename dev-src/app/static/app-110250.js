...
function allModels(includeUninstalled){
  ...
  (S.models||[]).forEach(function(model){
    ...
    if(['smollm2-135m','smollm2-360m','qwen2.5-0.5b','qwen2.5-coder-0.5b','tinyllama-1.1b','qwen3-4b','qwen3-8b','qwen2.5-coder-7b'].indexOf(id)>=0)return;
...
function modelsPage(){
  ...
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS intelligence</p><h2>Models</h2><p class="lead">Install VeloraOS Tiny, Quick or Deep Thoughts. Tiny and Quick use Qwen3 4B with different reasoning levels; Deep Thoughts uses Qwen3 8B at maximum Cosmic Reasoning. Automatic mode chooses between installed profiles for each question.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><h3>VeloraOS models</h3><div class="cards">'+branded+'</div>'+(other?'<h3 style="margin-top:28px">Specialist models</h3><div class="cards">'+other+'</div>':'');
}
function modelCard(model){
  ...
  return '<div class="model-card" id="model-'+escapeHtml(model.id)+'"><span class="tag '+(String(model.kind).indexOf('CPU')>=0?'':'warn')+'">'+escapeHtml(model.kind)+'</span><h3>'+escapeHtml(model.name)+'</h3><p class="muted">'+escapeHtml(model.tag)+'</p><div class="kv"><div><small>Download</small><b>'+escapeHtml(model.download)+'</b></div><div><small>Free space after</small><b>'+escapeHtml(availability.storage_after_install||'Unknown')+'</b></div></div>'+(task?'<div class="progress"><span style="width:'+progress+'%"></span></div><p class="small muted model-progress-copy">'+escapeHtml(progressText)+'</p>':'')+'<p class="small muted status-line">'+escapeHtml(statusText)+'</p><div class="model-actions">'+primary+remove+'</div></div>';
}
...
function normaliseWebMode(value){value=String(value||'auto').toLowerCase();return ['auto','on','off'].indexOf(value)>=0?value:'auto';}
function webModeName(){return S.webMode==='on'?'Web On':(S.webMode==='off'?'Web Off':'Web Auto');}
function cycleWebMode(){
  S.webMode=S.webMode==='auto'?'on':(S.webMode==='on'?'off':'auto');
  var chat=S.chats.find(function(item){return item.id===S.active;});
  if(chat){chat.webMode=S.webMode;chat.updatedAt=new Date().toISOString();saveChats();}
  var button=$('#web-mode-button');if(button)button.textContent=webModeName();
  announce(webModeName()+'. Web Auto searches when fresh information appears useful; Web On always searches; Web Off stays local.');
}
...
function emptyChat(){return '<div class="empty-chat"><div class="empty-orb" aria-hidden="true">'+icons.chat+'</div><h2>How can I help?</h2><p>Use VeloraOS Automatic for intelligent routing, or manually select Tiny, Quick or Deep Thoughts.</p></div>';}
...
function messageSourcesHtml(message){
  var sources=message&&message.sources;
  if(!Array.isArray(sources)||!sources.length)return '';
  return '<div class="web-sources"><div class="web-sources-title">Web sources</div>'+sources.map(function(source,i){
    var href=String(source.url||'');
    var title=String(source.title||source.source||href||('Source '+(i+1)));
    return '<a class="web-source" href="'+escapeHtml(href)+'" target="_blank" rel="noopener noreferrer"><span>['+(i+1)+']</span><b>'+escapeHtml(title)+'</b><small>'+escapeHtml(source.source||'')+'</small></a>';
  }).join('')+'</div>';
}
function messageHtml(message,index){var speaker=message.role==='user'?((S.user&&S.user.display_name)||'You'):'VeloraOS';var body=message.role==='assistant'?renderMarkdown(message.content||''):('<p>'+inlineMarkdown(message.content||'').replace(/\n/g,'<br>')+'</p>');return '<article class="msg" data-message-index="'+index+'" aria-label="Message from '+escapeHtml(speaker)+'">'+chatAvatar(message.role)+'<div class="msg-content"><div class="msg-role">'+escapeHtml(speaker)+'</div><div class="msg-body">'+imageStrip(message.images)+body+'</div>'+messageSourcesHtml(message)+messageStatsHtml(message)+messageActionsHtml(message,index)+'</div></article>';}
...
function newChat(){var id=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():String(Date.now());S.active=id;S.busy=false;S.chatScrollIntent='bottom';S.msgs=[];S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;S.chats=[{id:id,title:'New chat',model:S.selected,webMode:S.webMode,messages:[],createdAt:new Date().toISOString()}].concat(S.chats);saveChats();render();}
function normaliseChatModelSelection(id){
  var aliases={'tinyllama-1.1b':'veloraos-tiny','tinyllama:1.1b':'veloraos-tiny','qwen3-4b':'veloraos-quick','qwen3:4b':'veloraos-quick','veloraos-deep-thoughts':'veloraos-deep','smollm2-135m':'veloraos-auto','smollm2:135m':'veloraos-auto','smollm2-360m':'veloraos-auto','smollm2:360m':'veloraos-auto','qwen2.5-0.5b':'veloraos-auto','qwen2.5:0.5b':'veloraos-auto','qwen2.5-coder-0.5b':'veloraos-auto','qwen2.5-coder:0.5b':'veloraos-auto','llama3.2-1b':'veloraos-auto','llama3.2:1b':'veloraos-auto','qwen3-8b':'veloraos-auto','qwen3:8b':'veloraos-auto','qwen2.5-coder-7b':'veloraos-auto','qwen2.5-coder:7b':'veloraos-auto'};
  var value=String(id||'veloraos-auto');
  value=aliases[value]||value;
  return ['veloraos-auto','veloraos-tiny','veloraos-quick','veloraos-deep'].indexOf(value)>=0?value:'veloraos-auto';
}
function openChat(id){var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;S.chatScrollIntent='bottom';S.active=id;S.msgs=chat.messages||[];S.selected=normaliseChatModelSelection(chat.model||S.selected);S.webMode=normaliseWebMode(chat.webMode||'auto');S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;syncActiveChatBusy();render();}
...
function saveActive(){var chat=S.chats.find(function(item){return item.id===S.active;});if(!chat)return saveChats();chat.messages=S.msgs;chat.model=S.selected;chat.webMode=S.webMode;chat.updatedAt=new Date().toISOString();if(!chat.createdAt)chat.createdAt=chat.updatedAt;if(!chat.title||chat.title==='New chat'){chat.title=(S.msgs.find(function(message){return message.role==='user';})||{}).content||chat.title||'New chat';if(chat.title.length>42)chat.title=chat.title.slice(0,42);}return saveChats();}
...
async function performGeneration(){
  ...
  try{
    ...
    S.chatTasks[S.active]={id:start.task_id,task_id:start.task_id,kind:'chat',chat_id:S.active,status:start.status||'queued',model:start.model,selected_profile:start.selected_profile,selected_profile_name:start.selected_profile_name,route_reason:start.route_reason,web_mode:start.web_mode||S.webMode,web_used:!!start.web_planned,output:start.web_planned?'Preparing web search…':'Thinking…'};
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.25')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
...
window.Velora={
  go:go,installModel:installModel,cancelModelDownload:cancelModelDownload,deleteModel:deleteModel,refreshDiagnostics:refreshDiagnostics,runAccelerationTest:runAccelerationTest,newChat:newChat,openChat:openChat,deleteChat:deleteChat,renameChat:renameChat,exportChat:exportChat,copyMessage:copyMessage,copyCode:copyCode,beginEditMessage:beginEditMessage,cancelEditMessage:cancelEditMessage,regenerateMessage:regenerateMessage,setSearch:setSearch,toggleMobileChats:toggleMobileChats,toggleModelMenu:toggleModelMenu,selectModel:selectModel,cycleWebMode:cycleWebMode,send:send,stopGeneration:stopGeneration,toggleCosmicReasoning:toggleCosmicReasoning,previewCosmicReasoning:previewCosmicReasoning,commitCosmicReasoning:commitCosmicReasoning,
...
window.VELORAOS_RELEASE="1.10.25";
