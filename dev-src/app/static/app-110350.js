...
function modelsPage(){
  ...
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS intelligence</p><h2>Models</h2><p class="lead">Install VeloraOS Tiny, Quick or Deep Thoughts for general chat. Qwen3 4B Instruct is now the recommended reliability runtime for Tiny and Quick; VeloraOS uses it automatically once installed. VeloraOS Coding remains a separate hardware-aware programming option.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><h3>VeloraOS models</h3><div class="cards">'+branded+'</div>'+(other?'<h3 style="margin-top:28px">Specialist models</h3><div class="cards">'+other+'</div>':'');
}
...
function mobileChatActionsHtml(){
  var models=chatModelOptions().map(function(model){
    var selected=model.id===S.selected;
    return '<button class="mobile-chat-action mobile-model-action'+(selected?' selected':'')+'" onclick="Velora.mobileSelectModel(\''+escapeHtml(model.id)+'\')"><span>'+escapeHtml(model.name)+'</span><small>'+escapeHtml(model.kind)+'</small></button>';
  }).join('');
  return '<button id="mobile-chat-actions-backdrop" class="mobile-chat-actions-backdrop" type="button" onclick="Velora.toggleMobileChatActions(false)" '+(S.mobileChatActionsOpen?'':'hidden')+' aria-label="Close chat controls"></button>'+
    '<div id="mobile-chat-actions" class="mobile-chat-actions'+(S.mobileChatActionsOpen?' open':'')+'" role="dialog" aria-label="Chat controls">'+
      ...
      '<div class="mobile-chat-actions-head"><b>Chat controls</b><button onclick="Velora.toggleMobileChatActions(false)" aria-label="Close">&times;</button></div>'+
      '<div class="mobile-chat-action-grid">'+
        '<button class="mobile-chat-action" onclick="Velora.mobileCycleReasoning()"><span>Cosmic Reasoning</span><small>Power '+clampReasoning(S.reasoning)+' / 5</small></button>'+
        '<button class="mobile-chat-action'+(S.lockdownMode?' active':'')+'" onclick="Velora.mobileToggleLockdown()"><span>🛡 Lockdown</span><small>'+(S.lockdownMode?'Offline only':'Automatic web enabled')+'</small></button>'+
      ...
      '</div>'+
      '<div class="mobile-chat-models"><span class="mobile-chat-actions-label">Model · Automatic or manual</span>'+models+'</div>'+
    '</div>';
}
function chatPage(){
...
function mobileSelectModel(id){selectModel(id);toggleMobileChatActions(false);}
function mobileToggleLockdown(){toggleLockdownMode();toggleMobileChatActions(false);}
function mobileCycleReasoning(){
  var next=clampReasoning(S.reasoning)+1;
  if(next>5)next=1;
  commitCosmicReasoning(next);
  toggleMobileChatActions(false);
}
...
function newChat(){S.mobileChatActionsOpen=false;var id=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():String(Date.now());S.active=id;S.busy=false;S.chatScrollIntent='bottom';S.msgs=[];S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;S.chats=[{id:id,title:'New chat',model:S.selected,messages:[],createdAt:new Date().toISOString()}].concat(S.chats);saveChats();render();}
...
function openChat(id){S.mobileChatActionsOpen=false;var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;S.chatScrollIntent='bottom';S.active=id;S.msgs=chat.messages||[];S.selected=normaliseChatModelSelection(chat.model||S.selected);S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;syncActiveChatBusy();render();}
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.35')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
...
window.VELORAOS_RELEASE="1.10.35";
