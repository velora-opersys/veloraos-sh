...
var S={
  ...
  busy:false,error:'',search:'',pendingImages:[],user:null,settings:{},accounts:[],pendingAvatar:null,controller:null,stopRequested:false,reasoning:2,reasoningOpen:false,csrf:'',license:null,licenseBusy:false,showLicenseKey:false
};
...
async function logout(){
  ...
  S.user=null;S.csrf='';S.license=null;S.settings={};S.chats=[];S.msgs=[];S.active=null;showLogin('You have been signed out.');
}
async function startAuthenticated(user,csrfToken){
  S.user=user;S.csrf=csrfToken||S.csrf||'';S.pendingAvatar=null;
  try{await loadSettings();await loadCore();await loadLicense();}
