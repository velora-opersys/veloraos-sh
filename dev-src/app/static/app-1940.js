...
window.addEventListener('keydown',function(event){
  if(S&&S.mobileChatsOpen&&event.key==='Escape'){event.preventDefault();S.mobileChatsOpen=false;render();return;}
...
var S={
  ...
  backups:[],recovery:null,recoveryBusy:false,recoveryNotice:'',diagnosticsArchive:null,topMenuOpen:false,mobileChatsOpen:false
};
...
function top(){
  ...
  var updateButton=S.user&&S.user.role==='admin'?'<button class="'+(S.page==='updates'?'active':'')+'" onclick="Velora.go(\'updates\')">Updates'+updateBadge+'</button>':'';
  var recoveryButton=S.user&&S.user.role==='admin'?'<button class="'+(S.page==='recovery'?'active':'')+'" onclick="Velora.go(\'recovery\')">Recovery</button>':'';
  var meta=statusMeta();
  var statusClass=/CPU only/i.test(meta.main)?' status-chip-warn':'';
  return '<div class="top'+(maximum?' cosmic-maximum':'')+'"><div class="top-inner"><div class="brand"><div class="logo">V</div><div><h1>VeloraOS</h1><p>'+escapeHtml((S.user&&S.user.display_name)||'User')+'\'s local AI appliance</p></div></div><div class="top-center"><div id="cosmic-header-badge" class="cosmic-header-badge" '+(maximum?'':'hidden')+'><span>✦</span> Maximum Power</div><nav class="nav nav-dock" aria-label="Primary"><button class="'+(S.page==='home'?'active':'')+'" onclick="Velora.go(\'home\')">Home</button><button class="'+(S.page==='models'?'active':'')+'" onclick="Velora.go(\'models\')">Models</button><button class="'+(S.page==='diagnostics'?'active':'')+'" onclick="Velora.go(\'diagnostics\')">Diagnostics</button>'+recoveryButton+updateButton+'<button class="'+(S.page==='upgrades'?'active':'')+'" onclick="Velora.go(\'upgrades\')">Upgrades</button></nav></div><div class="top-right"><button class="status-chip'+statusClass+'" onclick="Velora.go(\'diagnostics\')" title="'+escapeHtml(status())+'"><span class="dot"></span><span class="status-chip-copy"><b>'+escapeHtml(meta.main)+'</b><small>'+escapeHtml(meta.sub)+'</small></span></button><div class="profile-menu-wrap"><button class="top-profile top-profile-trigger" onclick="Velora.toggleTopMenu(event)" aria-expanded="'+(S.topMenuOpen?'true':'false')+'" aria-haspopup="menu">'+avatarHtml(S.user,'top-avatar')+'<span>'+escapeHtml((S.user&&S.user.display_name)||'Profile')+'</span><span class="top-profile-chevron">▾</span></button><div class="profile-menu'+(S.topMenuOpen?' open':'')+'" role="menu"><button role="menuitem" onclick="Velora.go(\'settings\')">Settings</button><button role="menuitem" onclick="Velora.logout()">Logout</button></div></div></div></div></div>';
}
...
function chatPage(){
  ...
  var chatRows=filteredChats().map(function(chat){return '<div class="chat-row '+(S.active===chat.id?'active':'')+'"><button class="chat-item" onclick="Velora.openChat(\''+chat.id+'\')">'+escapeHtml(chat.title||'New chat')+'</button><button class="chat-delete" onclick="Velora.deleteChat(\''+chat.id+'\')" aria-label="Delete '+escapeHtml(chat.title||'chat')+'" title="Delete chat">&times;</button></div>';}).join('');
  var modelRows=allModels().map(function(model){return '<button class="model-option" onclick="Velora.selectModel(\''+escapeHtml(model.id)+'\')"><b>'+escapeHtml(model.name)+'</b><small>'+escapeHtml(model.kind)+'</small></button>';}).join('');
  var sidebar='<aside class="chat-sidebar'+(S.mobileChatsOpen?' open':'')+'" aria-label="Chat history"><div class="mobile-drawer-head"><b>Chats</b><button type="button" onclick="Velora.toggleMobileChats(false)" aria-label="Close chat history">&times;</button></div><button class="new-chat" onclick="Velora.newChat()">+ New chat</button><input class="search" placeholder="Search titles and messages" value="'+escapeHtml(S.search)+'" oninput="Velora.setSearch(this.value)"><div class="chat-list">'+chatRows+'</div></aside>';
...
function toggleMobileChats(open){S.mobileChatsOpen=typeof open==='boolean'?open:!S.mobileChatsOpen;S.menu=false;render();}
...
async function runSetupHardware(){S.setupBusy=true;setStatus('setup-action-status','Detecting hardware…',false);try{var result=await API.post('/api/setup/hardware-test',{});S.system=result.hardware;await refreshSetup();finishSetupTransition(5);}catch(error){S.setupBusy=false;setStatus('setup-action-status',humanError(error),true);}}
...
function recoveryPage(){
  ...
  return '<div class="section-head"><div><p class="eyebrow">Protection and support</p><h2>Backup & Recovery</h2><p class="lead">Verified backups, safe restoration and administrator recovery tools for VeloraOS.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div>'+
...
window.Velora={
  ...
  reload:reload,logout:logout,toggleTopMenu:toggleTopMenu,doLogin:doLogin,openImagePicker:openImagePicker,handleImageFiles:handleImageFiles,removePendingImage:removePendingImage,
