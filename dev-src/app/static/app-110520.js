...
async function go(page){if(page!=='chat')S.mobileChatActionsOpen=false;if(page==='knowledge'){await loadKnowledge();}
...
function systemHealthOverview(){
  ...
  return '<section class="system-appliance panel"><div class="system-appliance-head"><div><p class="eyebrow">'+escapeHtml(os.name||'VeloraOS 2.0')+'</p><h3>System</h3><p class="muted">Live appliance status and operating-system controls.</p></div><span class="diagnostic-badge '+(healthy?'ready':'not-ready')+'">'+(healthy?'Healthy':'Needs attention')+'</span></div>'+
    '<div class="system-appliance-grid">'+
      ...
      diagnosticFact('VeloraOS',sys.version||'1.10.52',os.build?('OS build '+os.build):'Application')+
...
function systemControlPanel(){
  var sys=S.system||{};var available=!!sys.controlsAvailable;var disabled=(S.systemActionBusy||!available)?' disabled':'';var reason=available?'Administrator controls are restricted to four allow-listed system actions.':'OS controls are available when this application is running on a VeloraOS OS installation with the system-control helper.';
...
window.VELORAOS_RELEASE="1.10.52";
