...
function syncVisualViewport(){
  var viewport=window.visualViewport;var height=viewport?viewport.height:window.innerHeight;
  var offsetTop=viewport?viewport.offsetTop:0;var keyboard=Math.max(0,window.innerHeight-height-offsetTop);
  document.documentElement.style.setProperty('--velora-vvh',Math.max(320,height)+'px');
  document.documentElement.style.setProperty('--velora-keyboard-inset',Math.max(0,keyboard)+'px');
...
function registerPwa(){
  ...
  document.addEventListener('focusin',function(event){if(!isStandalonePwa())return;var tag=String(event.target&&event.target.tagName||'').toLowerCase();if(['input','textarea','select'].indexOf(tag)>=0){setTimeout(function(){syncVisualViewport();try{event.target.scrollIntoView({block:'nearest',behavior:'smooth'});}catch(_){}},120);}});
...
function afterChat(previousViewport){
  var input=$('#prompt');
  if(input){
    input.addEventListener('input',function(){this.style.height='auto';this.style.height=Math.min(this.scrollHeight,180)+'px';if(S.editingIndex!==null)S.editingDraft=this.value;else{if(!S.chatDrafts)S.chatDrafts={};if(S.active)S.chatDrafts[S.active]=this.value;}});
...
function systemHealthOverview(){
  ...
  return '<section class="system-appliance panel"><div class="system-appliance-head"><div><p class="eyebrow">'+escapeHtml(os.name||'VeloraOS 2.0')+'</p><h3>System</h3><p class="muted">Live appliance status and operating-system controls.</p></div><span class="diagnostic-badge '+(healthy?'ready':'not-ready')+'">'+(healthy?'Healthy':'Needs attention')+'</span></div>'+
    '<div class="system-appliance-grid">'+
      ...
      diagnosticFact('VeloraOS',sys.version||'1.10.54',os.build?('OS build '+os.build):'Application')+
...
window.VELORAOS_RELEASE="1.10.54";
