...
function notificationItems(){
  ...
  if(S.user&&S.user.role==='admin'&&S.update){
    if(S.update.updateAvailable){
      var version=String(S.update.latestVersion||'update');
      items.push({id:'update-'+version,type:'update',title:S.update.title||('VeloraOS '+version+' available'),message:(S.update.releaseNotes&&S.update.releaseNotes[0])||'A newer VeloraOS release is ready to review.',action:'updates',actionLabel:'View update',time:S.update.publishedAt||S.update.lastCheckedAt||'',priority:'important'});
    }else if(S.update.state==='failed'){
      items.push({id:'update-failed-'+String(S.update.lastCheckedAt||'current'),type:'warning',title:'Update check needs attention',message:S.update.error||S.update.message||'VeloraOS could not complete the latest update check.',action:'updates',actionLabel:'Open updates',time:S.update.lastCheckedAt||'',priority:'warning'});
    }else if(['checking','downloading','installing'].indexOf(S.update.state)>=0){
      items.push({id:'update-active-'+String(S.update.latestVersion||'current'),type:'activity',title:'System update in progress',message:S.update.message||'VeloraOS is processing a system update.',action:'updates',actionLabel:'View progress',time:S.update.lastCheckedAt||'',priority:'activity'});
    }
  }
  return items;
}
...
function notificationsPage(){
  ...
  var cards=items.map(function(item){
    ...
    return '<article class="notification-card '+escapeHtml(item.priority||'')+'"><div class="notification-icon" aria-hidden="true">'+notificationIcon(item.type)+'</div><div class="notification-copy"><div class="notification-meta"><span class="tag">'+escapeHtml(item.type==='update'?'System update':item.type==='warning'?'Attention':'Activity')+'</span>'+(item.time?'<time>'+escapeHtml(formatUpdateDate(item.time))+'</time>':'')+'</div><h3>'+escapeHtml(item.title)+'</h3><p>'+escapeHtml(item.message)+'</p>'+action+'</div></article>';
  }).join('');
