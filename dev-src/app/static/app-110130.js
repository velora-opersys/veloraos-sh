  ...
  try{
    ...
    tasks.forEach(function(task){
      ...
      if(kind==='model'&&task.model_id){
        if(taskIsActive(task)||['error','failed','cancelled'].indexOf(state)>=0)nextModels[task.model_id]=task;
...
function allModels(includeUninstalled){
  var base=[
    {id:'veloraos-tiny',name:'VeloraOS Tiny',tag:'tinyllama:1.1b',kind:'VeloraOS Basic',category:'chat',download:'640 MB',desc:'Fast and lightweight for basic operations and short questions.'},
    {id:'veloraos-quick',name:'VeloraOS Quick',tag:'qwen3:4b',kind:'VeloraOS Everyday',category:'chat',download:'2.6 GB',desc:'Fast everyday assistance powered by Qwen3 4B.'},
    {id:'veloraos-deep',name:'VeloraOS Deep Thoughts',tag:'qwen3:4b',kind:'VeloraOS Reasoning',category:'chat',download:'2.6 GB',desc:'Qwen3 4B with deeper reasoning and analysis settings.'},
...
function modelsPage(){
  ...
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS intelligence</p><h2>Models</h2><p class="lead">Install VeloraOS Tiny, Quick or Deep Thoughts. Automatic mode chooses between installed profiles for each question.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><h3>VeloraOS models</h3><div class="cards">'+branded+'</div>'+(other?'<h3 style="margin-top:28px">Specialist models</h3><div class="cards">'+other+'</div>':'');
}
...
function newChat(){var id=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():String(Date.now());S.active=id;S.busy=false;S.msgs=[];S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;S.chats=[{id:id,title:'New chat',model:S.selected,messages:[],createdAt:new Date().toISOString()}].concat(S.chats);saveChats();render();}
...
function openChat(id){var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;S.active=id;S.msgs=chat.messages||[];S.selected=normaliseChatModelSelection(chat.model||S.selected);S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;syncActiveChatBusy();render();}
...
async function performGeneration(){
  ...
  S.busy=true;S.error='';S.chatNotice='VeloraOS is generating this response in the background. You can leave Chat or close the app.';render();
  try{
    ...
    S.chatTasks[S.active]={id:start.task_id,task_id:start.task_id,kind:'chat',chat_id:S.active,status:start.status||'queued',model:start.model,selected_profile:start.selected_profile,selected_profile_name:start.selected_profile_name,route_reason:start.route_reason};
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.13')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
...
window.VELORAOS_RELEASE="1.10.13";
