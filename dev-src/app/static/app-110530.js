...
var S={
  ...
  backups:[],recovery:null,recoveryBusy:false,recoveryNotice:'',diagnosticsArchive:null,systemActionBusy:false,systemActionNotice:'',topMenuOpen:false,mobileChatsOpen:false,a11yMessage:'',
...
function registerPwa(){
  ...
  window.addEventListener('popstate',function(event){var page=(event.state&&event.state.veloraPage)||currentLaunchPage()||'home';if(S.user){S.page=page;S.topMenuOpen=false;S.mobileChatsOpen=false;render();}});
...
function nativeAppBar(){
  var root=['home','chat','models','settings'].indexOf(S.page)>=0;var unread=unreadNotificationCount();
...
async function go(page){if(page!=='chat')S.mobileChatActionsOpen=false;hapticTick();if(page==='knowledge'){await loadKnowledge();}
...
function emptyChat(){return '<div class="empty-chat"><div class="empty-orb" aria-hidden="true">'+icons.chat+'</div><h2>How can I help?</h2><p>VeloraOS uses Qwen2.5 3B for normal chat. Turn on Coding when you want the hardware-aware coding model.</p></div>';}
...
function openChat(id){S.mobileChatActionsOpen=false;var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;S.chatScrollIntent='bottom';S.active=id;S.msgs=chat.messages||[];S.codingMode=chatCodingMode(chat);S.selected=S.codingMode?'veloraos-coding':'veloraos-main';S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;syncActiveChatBusy();render();}
...
function systemHealthOverview(){
  ...
  return '<section class="system-appliance panel"><div class="system-appliance-head"><div><p class="eyebrow">'+escapeHtml(os.name||'VeloraOS 2.0')+'</p><h3>System</h3><p class="muted">Live appliance status and operating-system controls.</p></div><span class="diagnostic-badge '+(healthy?'ready':'not-ready')+'">'+(healthy?'Healthy':'Needs attention')+'</span></div>'+
    '<div class="system-appliance-grid">'+
      ...
      diagnosticFact('VeloraOS',sys.version||'1.10.53',os.build?('OS build '+os.build):'Application')+
...
window.VELORAOS_RELEASE="1.10.53";
