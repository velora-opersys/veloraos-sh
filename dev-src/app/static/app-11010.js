...
async function go(page){
  ...
  if(['models','image','video','upgrades'].indexOf(page)>=0){S.page='coming-soon';S.comingSoonTitle={models:'Models',image:'Image Studio',video:'Video Studio',upgrades:'Upgrades'}[page];render();focusAfterRender('#main-content');return;}
  S.page=page;S.error='';
...
function homePage(){
  return updateNotificationHtml()+'<section class="hero"><p class="eyebrow">VeloraOS Public Beta</p><h2>Private AI chat, ready locally.</h2><p class="lead">Chat is available during the public beta. More private AI workspaces are being prepared and will unlock in future releases.</p></section><section class="apps beta-apps">'+appCard('chat','Chat','Talk to your installed local models with private saved conversations.',false)+appCard('models','Models','A full model library and management workspace.',true)+appCard('image','Image Studio','Private local image generation and galleries.',true)+appCard('video','Video Studio','Private local video generation on supported hardware.',true)+'</section><div id="update-live" class="sr-only" aria-live="polite"></div>';
}
function appCard(page,title,description,comingSoon){return '<button class="app-card'+(comingSoon?' coming-soon':'')+'" '+(comingSoon?'disabled aria-disabled="true"':'onclick="Velora.go(\''+page+'\')"')+'><div class="app-icon">'+icons[page]+'</div>'+(comingSoon?'<span class="beta-coming-soon">Coming soon</span>':'<span class="beta-live">Available now</span>')+'<h3>'+escapeHtml(title)+'</h3><p>'+escapeHtml(description)+'</p></button>';}
...
window.VELORAOS_RELEASE="1.10.1";
