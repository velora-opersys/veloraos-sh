...
var S={
  page:'home',system:null,models:[],selected:'veloraos-main',codingMode:false,menu:false,msgs:[],chats:[],active:null,
  busy:false,error:'',search:'',pendingImages:[],user:null,settings:{},accounts:[],pendingAvatar:null,controller:null,stopRequested:false,reasoning:2,reasoningOpen:false,csrf:'',license:null,licenseLocked:false,licenseLockMessage:'',licenseBusy:false,showLicenseKey:false,update:null,updateBusy:false,updateModal:false,updatePoll:null,updateAnnounced:'',setup:null,setupStep:1,setupBusy:false,setupDownload:null,setupDeviceDraft:null,setupModelSelection:null,setupComplete:false,modelTasks:{},chatTasks:{},backgroundPoll:null,backgroundSeen:{},mobileSecurity:null,chatScrollIntent:'',lockdownMode:false,
  diagnostics:null,diagnosticsBusy:false,diagnosticsTest:null,editingIndex:null,editingDraft:'',chatDrafts:{},chatNotice:'',
  backups:[],recovery:null,recoveryBusy:false,recoveryNotice:'',diagnosticsArchive:null,systemActionBusy:false,systemActionNotice:'',topMenuOpen:false,mobileChatsOpen:false,mobileConversationOpen:false,mobileAttachmentOpen:false,a11yMessage:'',
  knowledge:[],knowledgeResults:[],mobileChatActionsOpen:false,imageStudio:null,imageBusy:false,imageError:'',imageDraft:{prompt:'',negativePrompt:'',checkpoint:'',size:'768x768',steps:28,cfg:7,sampler:'euler',scheduler:'normal',seed:''}
};
...
var pwaControllerChanged=false;
var nativeConfirmResolve=null;
var nativePromptResolve=null;
var nativeViewportBaseline=0;
var nativeViewportWidth=0;
var nativeKeyboardWasOpen=false;
...
function hapticTick(){try{if(navigator.vibrate)navigator.vibrate(8);}catch(_){} }
function applyPwaMode(){
  document.documentElement.classList.toggle('velora-standalone',isStandalonePwa());
  document.documentElement.classList.toggle('velora-ios',isIosDevice());
  document.body&&document.body.classList.toggle('pwa-standalone',isStandalonePwa());
}
function activeEditableElement(){
  var active=document.activeElement;if(!active)return false;var tag=String(active.tagName||'').toLowerCase();return ['input','textarea','select'].indexOf(tag)>=0||active.isContentEditable===true;
}
function syncVisualViewport(){
  var viewport=window.visualViewport;
  var height=Math.max(320,viewport?viewport.height:window.innerHeight);
  var width=Math.max(240,viewport?viewport.width:window.innerWidth);
  var offsetTop=Math.max(0,viewport?viewport.offsetTop:0);
  var editable=activeEditableElement();
  if(!nativeViewportWidth||Math.abs(width-nativeViewportWidth)>80){nativeViewportWidth=width;nativeViewportBaseline=height;}
  if(!editable){nativeViewportBaseline=height;}else if(!nativeViewportBaseline){nativeViewportBaseline=Math.max(height,window.innerHeight||height);}
  var keyboard=Math.max(0,nativeViewportBaseline-height-offsetTop);
  var keyboardOpen=!!(isStandalonePwa()&&editable&&keyboard>80);
  document.documentElement.style.setProperty('--velora-vvh',height+'px');
  document.documentElement.style.setProperty('--velora-vv-top',offsetTop+'px');
  document.documentElement.style.setProperty('--velora-keyboard-inset',keyboard+'px');
  document.documentElement.classList.toggle('keyboard-open',keyboardOpen);
  if(document.body)document.body.classList.toggle('keyboard-open',keyboardOpen);
  if(keyboardOpen!==nativeKeyboardWasOpen){
    nativeKeyboardWasOpen=keyboardOpen;
    if(typeof S!=='undefined'&&S.page==='chat'){window.requestAnimationFrame(function(){scrollChat();});}
  }
}
function currentLaunchPage(){
  try{var page=new URLSearchParams(location.search).get('page');return ['home','chat','models','knowledge','settings','notifications','system-info','updates','diagnostics','recovery'].indexOf(page)>=0?page:'';}catch(_){return '';}
}
function syncPageUrl(page,replace){
  if(!history||!history.pushState)return;try{var url=new URL(location.href);url.searchParams.set('page',page||'home');url.searchParams.delete('system_action');var state={veloraPage:page||'home'};(replace?history.replaceState:history.pushState).call(history,state,'',url.pathname+url.search+url.hash);}catch(_){}
}
function nativeToast(message,kind){
  var host=document.getElementById('native-layer');if(!host)return;var old=host.querySelector('.native-toast');if(old)old.remove();
  var toast=document.createElement('div');toast.className='native-toast '+(kind||'');toast.setAttribute('role','status');toast.textContent=String(message||'');host.appendChild(toast);
  requestAnimationFrame(function(){toast.classList.add('show');});setTimeout(function(){toast.classList.remove('show');setTimeout(function(){toast.remove();},220);},2600);
}
function closeNativeConfirm(value){
  var host=document.getElementById('native-layer');var sheet=host&&host.querySelector('.native-confirm-backdrop');if(sheet)sheet.remove();
  var resolve=nativeConfirmResolve;nativeConfirmResolve=null;if(resolve)resolve(!!value);
}
function nativeConfirm(options){
  options=options||{};if(nativeConfirmResolve)closeNativeConfirm(false);hapticTick();
  return new Promise(function(resolve){
    nativeConfirmResolve=resolve;var host=document.getElementById('native-layer');if(!host){resolve(window.confirm(options.message||'Continue?'));nativeConfirmResolve=null;return;}
    var backdrop=document.createElement('div');backdrop.className='native-confirm-backdrop';backdrop.innerHTML='<div class="native-confirm-sheet" role="dialog" aria-modal="true" aria-labelledby="native-confirm-title"><div class="native-sheet-grabber" aria-hidden="true"></div><div class="native-confirm-copy"><h3 id="native-confirm-title">'+escapeHtml(options.title||'Confirm')+'</h3><p>'+escapeHtml(options.message||'Continue?')+'</p></div><div class="native-confirm-actions"><button class="native-sheet-button cancel" type="button" onclick="Velora.closeNativeConfirm(false)">'+escapeHtml(options.cancelLabel||'Cancel')+'</button><button class="native-sheet-button '+(options.destructive?'destructive':'primary')+'" type="button" onclick="Velora.closeNativeConfirm(true)">'+escapeHtml(options.confirmLabel||'Continue')+'</button></div></div>';
    backdrop.addEventListener('click',function(event){if(event.target===backdrop)closeNativeConfirm(false);});host.appendChild(backdrop);requestAnimationFrame(function(){backdrop.classList.add('open');var button=backdrop.querySelector('.native-sheet-button.primary,.native-sheet-button.destructive');if(button)button.focus();});
  });
}
function closeNativePrompt(save){
  var host=document.getElementById('native-layer');var backdrop=host&&host.querySelector('.native-prompt-backdrop');var input=backdrop&&backdrop.querySelector('.native-prompt-input');var value=save&&input?String(input.value||'').trim():null;
  if(backdrop)backdrop.remove();var resolve=nativePromptResolve;nativePromptResolve=null;if(resolve)resolve(value);
}
function nativePrompt(options){
  options=options||{};if(nativePromptResolve)closeNativePrompt(false);hapticTick();
  return new Promise(function(resolve){
    nativePromptResolve=resolve;var host=document.getElementById('native-layer');if(!host){resolve(window.prompt(options.title||'Enter a value',options.value||''));nativePromptResolve=null;return;}
    var backdrop=document.createElement('div');backdrop.className='native-prompt-backdrop';backdrop.innerHTML='<div class="native-prompt-sheet" role="dialog" aria-modal="true" aria-labelledby="native-prompt-title"><div class="native-sheet-grabber" aria-hidden="true"></div><div class="native-confirm-copy"><h3 id="native-prompt-title">'+escapeHtml(options.title||'Rename')+'</h3><p>'+escapeHtml(options.message||'')+'</p></div><input class="native-prompt-input" maxlength="80" autocomplete="off" value="'+escapeAttribute(options.value||'')+'" aria-label="'+escapeAttribute(options.inputLabel||options.title||'Name')+'"><div class="native-confirm-actions"><button class="native-sheet-button cancel" type="button" onclick="Velora.closeNativePrompt(false)">'+escapeHtml(options.cancelLabel||'Cancel')+'</button><button class="native-sheet-button primary" type="button" onclick="Velora.closeNativePrompt(true)">'+escapeHtml(options.confirmLabel||'Save')+'</button></div></div>';
    backdrop.addEventListener('click',function(event){if(event.target===backdrop)closeNativePrompt(false);});host.appendChild(backdrop);requestAnimationFrame(function(){backdrop.classList.add('open');var input=backdrop.querySelector('.native-prompt-input');if(input){input.focus();input.select();input.addEventListener('keydown',function(event){if(event.key==='Enter'){event.preventDefault();closeNativePrompt(true);}else if(event.key==='Escape'){event.preventDefault();closeNativePrompt(false);}});}});
  });
}
function pwaInstallState(){
  if(isStandalonePwa())return {state:'installed',title:'Installed',message:'VeloraOS is running as a full-screen Home Screen app.'};
  ...
  if(deferredPwaPrompt)return {state:'installable',title:'Ready to install',message:'Install VeloraOS for a standalone app experience.'};
  if(isIosDevice())return {state:'ios',title:'Add to Home Screen',message:'In Safari, tap Share, choose Add to Home Screen, then turn on Open as Web App when shown.'};
...
async function installPwa(){
  if(isStandalonePwa()){nativeToast('VeloraOS is already installed.','success');return;}
  if(!pwaSecureContext()){nativeToast('Trusted HTTPS is required before installation.','warning');return;}
  if(deferredPwaPrompt){deferredPwaPrompt.prompt();var result=await deferredPwaPrompt.userChoice;deferredPwaPrompt=null;nativeToast(result&&result.outcome==='accepted'?'VeloraOS installation accepted.':'Installation dismissed.',result&&result.outcome==='accepted'?'success':'');render();return;}
  if(isIosDevice()){
    await nativeConfirm({title:'Install VeloraOS',message:'In Safari, tap the Share button, choose Add to Home Screen, then confirm Add. Launch VeloraOS from the Home Screen for the native app layout.',confirmLabel:'Got it'});return;
  }
  nativeToast('Use your browser menu to install VeloraOS.');
}
function updateAppBadge(){
  var count=0;try{count=unreadNotificationCount();}catch(_){}
  try{if(count&&navigator.setAppBadge)navigator.setAppBadge(count);else if(!count&&navigator.clearAppBadge)navigator.clearAppBadge();}catch(_){}
}
function registerPwa(){
  applyPwaMode();syncVisualViewport();
  if(window.visualViewport){window.visualViewport.addEventListener('resize',syncVisualViewport);window.visualViewport.addEventListener('scroll',syncVisualViewport);}window.addEventListener('resize',syncVisualViewport);
  if(window.matchMedia){try{window.matchMedia('(display-mode: standalone)').addEventListener('change',function(){applyPwaMode();render();});}catch(_){} }
  if('serviceWorker' in navigator&&pwaSecureContext()){
    navigator.serviceWorker.register('/sw.js',{scope:'/'}).then(function(registration){
      registration.addEventListener('updatefound',function(){var worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',function(){if(worker.state==='installed'&&navigator.serviceWorker.controller){nativeToast('VeloraOS app shell updated.','success');}});});
    }).catch(function(error){console.warn('VeloraOS PWA registration failed',error);});
    navigator.serviceWorker.addEventListener('controllerchange',function(){if(pwaControllerChanged)return;pwaControllerChanged=true;setTimeout(function(){pwaControllerChanged=false;},1500);});
  }
  window.addEventListener('beforeinstallprompt',function(event){event.preventDefault();deferredPwaPrompt=event;render();});
  window.addEventListener('appinstalled',function(){deferredPwaPrompt=null;pushNotification({id:'pwa-installed-'+Date.now(),type:'system',title:'VeloraOS installed',message:'VeloraOS was added to this device.',priority:'success'});nativeToast('VeloraOS added to your Home Screen.','success');render();});
  window.addEventListener('popstate',function(event){var page=(event.state&&event.state.veloraPage)||currentLaunchPage()||'home';if(S.user){S.page=page;S.topMenuOpen=false;S.mobileChatsOpen=false;S.mobileConversationOpen=false;S.mobileAttachmentOpen=false;render();}});
  document.addEventListener('focusin',function(event){if(!isStandalonePwa())return;var tag=String(event.target&&event.target.tagName||'').toLowerCase();if(['input','textarea','select'].indexOf(tag)>=0){setTimeout(function(){syncVisualViewport();if(event.target&&event.target.id==='prompt'){scrollChat();syncComposerVisualState(event.target);}else{try{event.target.scrollIntoView({block:'nearest',behavior:'smooth'});}catch(_){}}},90);}});
  document.addEventListener('focusout',function(){if(!isStandalonePwa())return;setTimeout(function(){syncVisualViewport();},180);});
  window.addEventListener('orientationchange',function(){nativeViewportBaseline=0;nativeViewportWidth=0;setTimeout(syncVisualViewport,180);});
  var edgeGesture=null;document.addEventListener('touchstart',function(event){if(!isStandalonePwa()||S.page!=='chat'||!event.touches||event.touches.length!==1)return;var touch=event.touches[0];if(S.mobileChatsOpen||touch.clientX<=24)edgeGesture={x:touch.clientX,y:touch.clientY,drawer:!!S.mobileChatsOpen};},{passive:true});
  document.addEventListener('touchend',function(event){if(!edgeGesture||!event.changedTouches||!event.changedTouches.length)return;var touch=event.changedTouches[0];var dx=touch.clientX-edgeGesture.x;var dy=Math.abs(touch.clientY-edgeGesture.y);var wasOpen=edgeGesture.drawer;edgeGesture=null;if(dy>70)return;if(!wasOpen&&dx>72)toggleMobileChats(true);else if(wasOpen&&dx<-72)toggleMobileChats(false);},{passive:true});
}
...
async function loadSettings(){
  ...
  S.memoryEnabled=!!S.settings.memory_enabled;
  try{S.memory=await API.get('/api/memory');}catch(_){S.memory={enabled:S.memoryEnabled,items:[],count:0};}
  applyPersonalisation();
}

function syncLicenseLockState(){
  var state=String((S.license&&S.license.status)||'').toLowerCase();
  S.licenseLocked=!!(S.license&&!S.license.activated&&['inactive','expired','revoked','invalid','device_mismatch','device_limit'].indexOf(state)>=0);
  if(!S.licenseLocked){S.licenseLockMessage='';return;}
  if(state==='revoked')S.licenseLockMessage='This VeloraOS licence has been revoked. Activate a valid licence to restore access.';
  else if(state==='expired')S.licenseLockMessage='This VeloraOS licence has expired. Renew or activate a valid licence to restore access.';
  else if(state==='device_mismatch')S.licenseLockMessage='This activation belongs to a different device identity. Restore the original identity or activate a valid licence.';
  else if(state==='device_limit')S.licenseLockMessage='This licence has reached its device limit. Deactivate another device online, then retry.';
  else if(state==='invalid')S.licenseLockMessage='The configured licence is invalid. Activate a valid licence to restore access.';
  else S.licenseLockMessage='This VeloraOS licence is inactive. Activate a valid licence to restore access.';
}
async function loadLicense(){
  try{S.license=await API.get('/api/license/status');syncLicenseLockState();}
  catch(error){
    if(error.status===401)throw error;
    S.license={configured:false,activated:false,status:'error',connectionState:'error',message:humanError(error)};
    S.licenseLocked=false;S.licenseLockMessage='';
  }
}
...
function chatNeedsBackgroundNotification(task){
  if(!task||!task.chat_id)return false;
  // No notification when the answer is already visible in the chat the user
  // is actively looking at. Notifications are for genuinely background work.
  var sameActiveChat=S.page==='chat'&&String(S.active||'')===String(task.chat_id||'');
  var pageVisible=typeof document==='undefined'||!document.hidden;
  return !(sameActiveChat&&pageVisible);
}
function pruneLegacyChatCompletionNotifications(){
  var migrationKey='velora_notifications_quiet_chat_v11041';
  if(localStorage.getItem(migrationKey)==='1')return;
  var items=notificationHistory().filter(function(item){
    var id=String(item&&item.id||'');
    return id.indexOf('background-chat-complete-')!==0&&id.indexOf('background-chat-failed-')!==0;
  });
...
function pushNotification(item){
  ...
  item.time=item.time||new Date().toISOString();items.unshift(item);saveNotificationHistory(items);localStorage.removeItem(notificationReadKey(item.id));setTimeout(updateAppBadge,0);
}
function notificationItems(){
  pruneLegacyChatCompletionNotifications();
...
function markNotificationRead(id){if(id)localStorage.setItem(notificationReadKey(id),'1');setTimeout(updateAppBadge,0);}
function markAllNotificationsRead(){notificationItems().forEach(function(item){markNotificationRead(item.id);});updateAppBadge();render();announce('All notifications marked as read.');}
function clearNotifications(){saveNotificationHistory([]);updateAppBadge();render();announce('Notification history cleared.');}
...
async function loadUpdateStatus(){
  ...
  try{
    ...
    S.update=payload;
    if(S.update&&S.update.state==='complete'&&!S.update.rebootRequired){
      S.updateInstallStartedAt=0;
      S.update.transportReconnecting=false;
      S.update.message='Update complete. VeloraOS is ready.';
    }
    if(recovered&&S.update){
      S.update.transportReconnecting=false;
      if(S.update.state==='complete'){
        S.updateInstallStartedAt=0;
        S.update.message='Update complete. VeloraOS is ready.';
        announceUpdate('VeloraOS update completed and the Web UI reconnected.');
        var completedVersion=String(S.update.installedVersion||S.update.latestVersion||'');
        var reloadKey='velora_update_reloaded_'+completedVersion;
        if(completedVersion&&sessionStorage.getItem(reloadKey)!=='1'){
          sessionStorage.setItem(reloadKey,'1');
          setTimeout(function(){window.location.replace('/app?updated='+encodeURIComponent(completedVersion)+'&t='+Date.now());},450);
        }
      }else if(S.update.state==='failed'){
        S.updateInstallStartedAt=0;
        announceUpdate('VeloraOS reconnected after the update attempt. Review the recorded update log.');
...
async function refreshChatsFromServer(){
  ...
  if(current){S.msgs=current.messages||[];S.codingMode=chatCodingMode(current);S.selected=S.codingMode?'veloraos-coding':'veloraos-main';}
}
...
function taskResponseAlreadyPresent(chat,task){
  var messages=(chat&&chat.messages)||[];
  var id=String(task.id||task.task_id||'');
  return messages.some(function(message){
    return message&&message.role==='assistant'&&(
      String(message.taskId||'')===id||
      (task.response&&String(message.content||'')===String(task.response||''))
    );
  });
}
function canRecoverTaskIntoChat(chat,task){
  if(!chat||!task||!task.response)return false;
  var messages=chat.messages||[];
  if(taskResponseAlreadyPresent(chat,task))return false;
  var turnId=String(task.in_reply_to||'');
  if(turnId){
    var index=-1;
    messages.forEach(function(message,i){
      if(message&&message.role==='user'&&String(message.id||'')===turnId)index=i;
    });
    if(index<0)return false;
    for(var i=index+1;i<messages.length;i++){
      if(messages[i]&&messages[i].role==='user')return false;
      if(messages[i]&&messages[i].role==='assistant')return false;
    }
    return true;
  }
  // Legacy fallback only when the chat currently ends on an unanswered user.
  return !!(messages.length&&messages[messages.length-1]&&messages[messages.length-1].role==='user');
}
async function recoverCompletedChatResponses(tasks){
  var changed=false;
  (tasks||[]).forEach(function(task){
    if(String(task.kind||'')!=='chat'||!taskIsComplete(task)||!task.response)return;
    var chat=S.chats.find(function(item){return item.id===task.chat_id;});
    if(!canRecoverTaskIntoChat(chat,task))return;
    chat.messages=chat.messages||[];
    chat.messages.push({
      id:'assistant-'+String(task.id||task.task_id||newMessageId('task')),
      taskId:String(task.id||task.task_id||''),
      inReplyTo:String(task.in_reply_to||''),
      role:'assistant',
      content:String(task.response||''),
      stats:task.stats||{},
  ...
  if(changed){
    var current=S.chats.find(function(chat){return chat.id===S.active;});
    if(current)S.msgs=current.messages||[];
    await saveChats();
  }
  return changed;
}
  ...
  try{
    ...
    tasks.forEach(function(task){
      ...
      if(kind==='chat'&&task.chat_id){
        ...
        if(taskIsComplete(task)&&task.response)completedChatTasks.push(task);
        if(!S.backgroundSeen[id]&&taskIsComplete(task)){
          S.backgroundSeen[id]=true;
          refreshChats=true;
          if(task.chat_id===S.active)S.chatNotice='';
        }else if(!S.backgroundSeen[id]&&['error','failed'].indexOf(state)>=0){
          S.backgroundSeen[id]=true;
          var activeVisibleChat=S.page==='chat'&&String(S.active||'')===String(task.chat_id||'')&&(typeof document==='undefined'||!document.hidden);
          if(task.chat_id===S.active){
            S.error=task.error||task.output||'VeloraOS could not finish the response.';
            S.chatNotice='The local model stopped responding. Chat has been unlocked so you can retry.';
          }
        }
      }
    });
    ...
    var recoveredResponses=await recoverCompletedChatResponses(completedChatTasks);
    ...
    if(S.page==='chat'&&(refreshChats||recoveredResponses||busyChanged||(chatChanged&&!S.busy)))render();
    else if(refreshChats||refreshModels||recoveredResponses)render();
  }catch(error){
...
function top(){
  ...
  return '<header class="top'+(maximum?' cosmic-maximum':'')+'"><div class="top-inner"><div class="brand"><div class="logo" role="img" aria-label="VeloraOS">V</div><div><h1>VeloraOS</h1><p>'+escapeHtml((S.user&&S.user.display_name)||'User')+'\'s local AI appliance</p></div></div><div class="top-center"><div id="cosmic-header-badge" class="cosmic-header-badge" role="status" '+(maximum?'':'hidden')+'><span aria-hidden="true">&#10022;</span> Maximum Power</div><nav class="nav nav-dock" aria-label="Primary navigation">'+navButton('home','Home')+navButton('chat','Chat')+navButton('models','Models')+navButton('knowledge','Knowledge')+'</nav></div><div class="top-right"><button class="notification-button'+(notificationCount?' has-unread':'')+'" data-page="notifications" onclick="Velora.go(this.dataset.page)" aria-label="Notifications'+(notificationCount?': '+notificationCount+' unread':'')+'" title="Notifications"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg>'+notificationBadge+'</button><button class="status-chip'+statusClass+'" onclick="Velora.go(\'diagnostics\')" aria-label="System status: '+escapeHtml(meta.main)+'. '+escapeHtml(meta.sub)+'" title="'+escapeHtml(status())+'"><span class="dot" aria-hidden="true"></span><span class="status-chip-copy"><b>'+escapeHtml(meta.main)+'</b><small>'+escapeHtml(meta.sub)+'</small></span></button><div class="profile-menu-wrap"><button id="profile-menu-button" class="top-profile top-profile-trigger" onclick="Velora.toggleTopMenu(event)" aria-label="Open profile menu for '+escapeHtml((S.user&&S.user.display_name)||'Profile')+'" aria-expanded="'+(S.topMenuOpen?'true':'false')+'" aria-controls="profile-menu" aria-haspopup="menu">'+avatarHtml(S.user,'top-avatar')+'<span>'+escapeHtml((S.user&&S.user.display_name)||'Profile')+'</span><span class="top-profile-chevron" aria-hidden="true">&#9662;</span></button><div id="profile-menu" class="profile-menu'+(S.topMenuOpen?' open':'')+'" role="menu" aria-labelledby="profile-menu-button"><button role="menuitem" onclick="Velora.go(\'settings\')">Settings</button>'+adminItems+'<button role="menuitem" onclick="Velora.logout()">Logout</button></div></div></div></div></header>';
}
function nativePageTitle(){var titles={home:'VeloraOS',chat:'Chat',models:'Models',knowledge:'Knowledge',settings:'Settings',notifications:'Notifications','system-info':'System',updates:'Updates',diagnostics:'Diagnostics',recovery:'Recovery'};return titles[S.page]||S.comingSoonTitle||'VeloraOS';}
function nativeAppBar(){
  var unread=unreadNotificationCount();
  if(S.page==='chat'){
    var subtitle=S.codingMode?'Coding mode':'Private local AI';
    return '<header class="native-app-bar native-chat-bar"><button id="native-chat-drawer-button" class="native-bar-button" type="button" onclick="Velora.toggleMobileChats(true)" aria-label="Open conversations" aria-controls="chat-history-drawer" aria-expanded="'+(S.mobileChatsOpen?'true':'false')+'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h11"/></svg></button><div class="native-bar-title"><strong>'+escapeHtml(currentTitle())+'</strong><small>'+escapeHtml(subtitle)+'</small></div><button id="native-chat-more-button" class="native-bar-button" type="button" onclick="Velora.toggleMobileConversationMenu()" aria-label="Conversation actions" aria-haspopup="dialog" aria-expanded="'+(S.mobileConversationOpen?'true':'false')+'"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1.2"/><circle cx="12" cy="12" r="1.2"/><circle cx="19" cy="12" r="1.2"/></svg></button></header>';
  }
  var root=['home','models','settings'].indexOf(S.page)>=0;
  return '<header class="native-app-bar"><button class="native-bar-button '+(root?'brand-button':'')+'" type="button" onclick="Velora.'+(root?"go('home')":"goBack()")+'" aria-label="'+(root?'VeloraOS Home':'Back')+'">'+(root?'<span class="native-mini-logo">V</span>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 18l-6-6 6-6"/></svg>')+'</button><div class="native-bar-title"><strong>'+escapeHtml(nativePageTitle())+'</strong><small>'+escapeHtml(isStandalonePwa()?'Private local AI':'VeloraOS')+'</small></div><button class="native-bar-button" type="button" onclick="Velora.go(\'notifications\')" aria-label="Notifications'+(unread?': '+unread+' unread':'')+'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg>'+(unread?'<span class="native-badge">'+Math.min(99,unread)+'</span>':'')+'</button></header>';
}
function shell(content){return '<div class="shell">'+accessibilityChrome()+top()+nativeAppBar()+'<main id="main-content" class="main native-page" tabindex="-1">'+content+'</main>'+mobileBottomNav()+'</div>';}
function goBack(){if(history&&history.length>1){history.back();return;}go('home');}
async function go(page){if(page!=='chat')S.mobileChatActionsOpen=false;S.mobileConversationOpen=false;S.mobileAttachmentOpen=false;hapticTick();if(page==='knowledge'){await loadKnowledge();}
  ...
  if(page==='system-info'&&S.user&&S.user.role==='admin'){try{S.system=await API.get('/api/system');}catch(_){}await loadDiagnostics();await loadRecovery();await loadMobileSecurity();}
  ...
  syncPageUrl(S.page,false);render();focusAfterRender('#main-content');updateAppBadge();
}
function render(){
  var app=$('#app');if(!app)return;
  var activePrompt=(S.page==='chat')?$('#prompt'):null;
  var restorePromptFocus=!!(activePrompt&&document.activeElement===activePrompt);
  var restorePromptStart=restorePromptFocus?activePrompt.selectionStart:null;
  var restorePromptEnd=restorePromptFocus?activePrompt.selectionEnd:null;
  if(activePrompt&&S.editingIndex===null&&S.active){
    if(!S.chatDrafts)S.chatDrafts={};
    S.chatDrafts[S.active]=activePrompt.value;
  }
  var previousChatViewport=S.page==='chat'?captureChatViewport():null;
  if(!S.user){showLogin();return;}
  if(S.licenseLocked&&S.page!=='settings'){app.innerHTML=shell(licenseLockPage());return;}
  if(S.page==='setup'&&S.user&&S.user.role==='admin'){app.innerHTML=setupPage();afterSetupRender();return;}
  if(S.page==='chat'){
    app.innerHTML=chatPage();
    afterChat(previousChatViewport);
    if(restorePromptFocus){
      var restored=$('#prompt');
      if(restored){
        restored.focus();
        try{restored.setSelectionRange(restorePromptStart,restorePromptEnd);}catch(_){}
      }
    }
    return;
  }
  if(S.page==='knowledge'){app.innerHTML=shell(knowledgePage());return;}
...
function allModels(includeUninstalled){
  var base=[
    {id:'veloraos-main',name:'VeloraOS Main',tag:'qwen2.5:3b',kind:'Main assistant',category:'chat',download:'1.9 GB',desc:'Fast Qwen2.5 3B runtime for normal VeloraOS chat.'},
  ...
  (S.models||[]).forEach(function(model){
    ...
    if(['smollm2-135m','smollm2-360m','qwen2.5-0.5b','tinyllama-1.1b','qwen3-4b','qwen3-8b','qwen3-4b-instruct','veloraos-tiny','veloraos-quick','veloraos-deep'].indexOf(id)>=0)return;
...
function chatModelOptions(){
  var main=allModels(false).find(function(model){return model.id==='veloraos-main';});
  var codingInstalled=allModels(false).filter(function(model){return model.category==='coding';});
  return [
    {id:'veloraos-main',name:'VeloraOS',tag:'qwen2.5:3b',kind:'Qwen2.5 3B',category:'chat',availability:{installed:!!main}},
    {id:'veloraos-coding',name:'Coding',tag:'hardware-selected',kind:(codingInstalled.length?'Best installed coder for this hardware':'Hardware-aware coding'),category:'coding',availability:{installed:!!main||codingInstalled.length>0}}
  ];
}

async function loadKnowledge(){
  try{var payload=await API.get('/api/knowledge');S.knowledge=payload.documents||[];}
  catch(error){S.knowledge=[];}
}
async function uploadKnowledge(){
  var input=$('#knowledge-upload');
...
async function deleteKnowledge(id){
  if(!confirm('Remove this document from Knowledge?'))return;
  try{
    var response=await fetch('/api/knowledge/'+encodeURIComponent(id),{method:'DELETE',credentials:'same-origin',headers:{'X-CSRF-Token':API.csrf||''}});
    if(!response.ok)throw new Error(await response.text());
    await loadKnowledge();
    S.knowledgeResults=(S.knowledgeResults||[]).filter(function(item){return item.id!==id;});
    render();
  }catch(error){alert(humanError(error));}
}
async function searchKnowledge(){
  var input=$('#knowledge-query');
  var query=input&&input.value.trim();
  if(!query)return;
  try{
    var payload=await API.post('/api/knowledge/search',{query:query,limit:8});
    S.knowledgeResults=payload.results||[];
    render();
    setTimeout(function(){var field=$('#knowledge-query');if(field)field.value=query;},0);
  }catch(error){alert(humanError(error));}
}
function knowledgePage(){
  var docs=(S.knowledge||[]).map(function(item){
    return '<article class="knowledge-card"><div class="knowledge-card-icon" aria-hidden="true">K</div><div class="knowledge-card-copy"><b>'+escapeHtml(item.name)+'</b><small>'+Number(item.characters||0).toLocaleString()+' characters</small></div><button class="knowledge-delete" onclick="Velora.deleteKnowledge(\''+escapeHtml(item.id)+'\')" aria-label="Delete '+escapeHtml(item.name)+'">&times;</button></article>';
  }).join('');
  var results=(S.knowledgeResults||[]).map(function(item){
    return '<article class="knowledge-result-card"><div class="knowledge-result-meta"><span class="knowledge-dot"></span><b>'+escapeHtml(item.name)+'</b></div><p>'+escapeHtml(item.snippet||'')+'</p></article>';
  }).join('');
  return '<div class="section-head"><div><p class="eyebrow">Private local context</p><h2>Knowledge</h2><p class="lead">Keep manuals, notes, source files and documents on this VeloraOS device and search them locally.</p></div><label class="btn knowledge-add">Add document<input id="knowledge-upload" type="file" hidden onchange="Velora.uploadKnowledge()"></label></div>'+
    '<section class="knowledge-hero"><div class="knowledge-orb" aria-hidden="true">K</div><div><h3>Your private knowledge stays here</h3><p>Searches run locally on this appliance. Nothing from Knowledge is sent to the web search service.</p></div></section>'+
    '<section class="knowledge-search-panel"><div class="knowledge-search"><input id="knowledge-query" placeholder="Search your Knowledge library…"><button onclick="Velora.searchKnowledge()">Search</button></div><div class="knowledge-results">'+(results||'<div class="knowledge-empty">Search your documents to surface relevant excerpts.</div>')+'</div></section>'+
    '<section class="knowledge-library"><div class="section-head"><div><h3>Library</h3><p class="muted">'+(S.knowledge||[]).length+' document'+((S.knowledge||[]).length===1?'':'s')+'</p></div></div><div class="knowledge-grid">'+(docs||'<div class="knowledge-empty">No documents yet. Add a file to start your private library.</div>')+'</div></section>';
}
function modelsPage(){
  ...
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS intelligence</p><h2>Models</h2><p class="lead">VeloraOS Main uses Qwen2.5 3B for normal chat. Install coding models separately; the Coding switch automatically chooses the strongest sensible installed coder for this hardware.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><h3>VeloraOS models</h3><div class="cards">'+branded+'</div>'+(other?'<h3 style="margin-top:28px">Specialist models</h3><div class="cards">'+other+'</div>':'');
}
...
function normaliseVisibleAnswerUnit(value){
...
function dedupeRepeatedAnswerForDisplay(value){
  var text=String(value||'').trim();
  if(!text)return '';
  var raw=text.split(/\n\s*\n/);
  var kept=[];
  raw.forEach(function(unit){
    var norm=normaliseVisibleAnswerUnit(unit);
    if(norm.length<24){kept.push(unit);return;}
    var duplicate=kept.some(function(previous){
      return normaliseVisibleAnswerUnit(previous)===norm;
    });
    if(!duplicate)kept.push(unit);
  });
  return kept.join('\n\n').trim();
}
function serializableMessages(){return S.msgs.map(function(message){var content=message.role==='assistant'?dedupeRepeatedAnswerForDisplay(stripHiddenReasoningForDisplay(message.content||'')):(message.content||'');return {id:message.id||'',taskId:message.taskId||'',inReplyTo:message.inReplyTo||'',role:message.role,content:content,images:(message.images||[]).map(function(image){if(typeof image==='string')return {dataUrl:image,name:'image'};return {dataUrl:image.dataUrl||image.url||image.data||'',name:image.name||'image'};})};});}
function normaliseImageForChat(file){
  return new Promise(function(resolve,reject){
    if(!file||!file.type||file.type.indexOf('image/')!==0){reject(new Error('That file is not an image.'));return;}
    if(file.size>8*1024*1024){reject(new Error(file.name+' is over 8 MB.'));return;}
    var reader=new FileReader();
    reader.onerror=function(){reject(new Error('Could not read '+file.name+'.'));};
    reader.onload=function(){
      var source=String(reader.result||'');
      var image=new Image();
      image.onerror=function(){reject(new Error(file.name+' could not be decoded as an image.'));};
      image.onload=function(){
        try{
          var maxSide=2048;
          var width=image.naturalWidth||image.width;
          var height=image.naturalHeight||image.height;
          if(!width||!height){reject(new Error(file.name+' has invalid dimensions.'));return;}
          ...
          if(!ctx){reject(new Error('Image conversion is unavailable in this browser.'));return;}
          ctx.drawImage(image,0,0,outW,outH);
          var preserveAlpha=file.type==='image/png';
          var mime=preserveAlpha?'image/png':'image/jpeg';
          var dataUrl=canvas.toDataURL(mime,preserveAlpha?undefined:0.92);
          if(!dataUrl||dataUrl.indexOf('data:image/')!==0){reject(new Error('Image conversion failed.'));return;}
          resolve({name:file.name,type:mime,size:file.size,dataUrl:dataUrl,width:outW,height:outH});
...
function handleImageFiles(fileList){
  ...
  files.slice(0,4-S.pendingImages.length).forEach(function(file){
    normaliseImageForChat(file).then(function(image){
      S.pendingImages.push(image);
      render();
      var prompt=$('#prompt');if(prompt)prompt.focus();
...
function mobileChatActionsHtml(){
  var modeLabel=S.codingMode?'Coding mode':'Normal chat';
  return '<button id="mobile-chat-actions-backdrop" class="mobile-chat-actions-backdrop" type="button" onclick="Velora.toggleMobileChatActions(false)" '+(S.mobileChatActionsOpen?'':'hidden')+' aria-label="Close chat controls"></button>'+
    '<div id="mobile-chat-actions" class="mobile-chat-actions'+(S.mobileChatActionsOpen?' open':'')+'" role="dialog" aria-label="Chat controls">'+
      '<div class="mobile-chat-actions-handle"></div>'+
      '<div class="mobile-chat-actions-head"><div><b>Chat controls</b><small>'+escapeHtml(modeLabel)+' · Qwen2.5 3B</small></div><button onclick="Velora.toggleMobileChatActions(false)" aria-label="Close">&times;</button></div>'+
      '<div class="mobile-slider-card">'+
        '<div class="mobile-slider-head"><span>Cosmic Reasoning</span><b id="mobile-reasoning-value">'+clampReasoning(S.reasoning)+'</b></div>'+
        '<input id="mobile-reasoning-slider" class="mobile-reasoning-slider" type="range" min="1" max="5" step="1" value="'+clampReasoning(S.reasoning)+'" oninput="Velora.previewMobileReasoning(this.value)" onchange="Velora.commitMobileReasoning(this.value)">'+
        '<div class="mobile-slider-scale"><span>Fast</span><span>Maximum</span></div>'+
      '</div>'+
      '<div class="mobile-toggle-list">'+
        '<button class="mobile-toggle-row" onclick="Velora.toggleCodingMode()"><span><b>Coding mode</b><small>Use the best installed coding model for this hardware</small></span><span class="velora-switch '+(S.codingMode?'on':'')+'" aria-hidden="true"><i></i></span></button>'+
        '<button class="mobile-toggle-row" onclick="Velora.mobileToggleLockdown()"><span><b>🛡 Lockdown</b><small>Web Intelligence '+(S.lockdownMode?'off':'automatic')+'</small></span><span class="velora-switch '+(S.lockdownMode?'on':'')+'" aria-hidden="true"><i></i></span></button>'+
      '</div>'+
      '<div class="mobile-chat-action-grid">'+
        '<button class="mobile-chat-action" onclick="Velora.exportChat(\'markdown\');Velora.toggleMobileChatActions(false)"><span>Export Markdown</span><small>.md</small></button>'+
        '<button class="mobile-chat-action" onclick="Velora.exportChat(\'json\');Velora.toggleMobileChatActions(false)"><span>Export JSON</span><small>.json</small></button>'+
      '</div>'+
    '</div>';
}
function mobileConversationMenuHtml(){
  return '<button class="native-action-backdrop" type="button" onclick="Velora.toggleMobileConversationMenu(false)" '+(S.mobileConversationOpen?'':'hidden')+' aria-label="Close conversation actions"></button><div class="native-action-sheet'+(S.mobileConversationOpen?' open':'')+'" role="dialog" aria-label="Conversation actions"><div class="native-sheet-grabber" aria-hidden="true"></div><div class="native-action-heading"><b>'+escapeHtml(currentTitle())+'</b><small>Conversation</small></div><button type="button" onclick="Velora.newChat()"><span>New chat</span><small>Start a fresh conversation</small></button><button type="button" onclick="Velora.renameChat();Velora.toggleMobileConversationMenu(false)"><span>Rename</span><small>Change this conversation title</small></button><button type="button" onclick="Velora.openMobileChatControls()"><span>Chat controls</span><small>Coding, reasoning, Lockdown and export</small></button><button type="button" onclick="Velora.go(\'notifications\')"><span>Notifications</span><small>Background responses and system notices</small></button><button class="destructive" type="button" onclick="Velora.deleteCurrentChatFromMenu()"><span>Delete chat</span><small>This cannot be undone</small></button><button class="native-action-cancel" type="button" onclick="Velora.toggleMobileConversationMenu(false)">Cancel</button></div>';
}
function mobileAttachmentSheetHtml(){
  return '<button class="native-action-backdrop attachment-backdrop" type="button" onclick="Velora.toggleMobileAttachmentSheet(false)" '+(S.mobileAttachmentOpen?'':'hidden')+' aria-label="Close attachment options"></button><div class="native-action-sheet attachment-sheet'+(S.mobileAttachmentOpen?' open':'')+'" role="dialog" aria-label="Add to message"><div class="native-sheet-grabber" aria-hidden="true"></div><div class="native-action-heading"><b>Add to message</b><small>Images stay on your VeloraOS appliance</small></div><button type="button" onclick="Velora.choosePhotoLibrary()"><span>Photo Library</span><small>Choose one or more images</small></button><button type="button" onclick="Velora.chooseCamera()"><span>Take Photo</span><small>Use this device camera</small></button><button type="button" onclick="Velora.openKnowledgeFromComposer()"><span>Knowledge</span><small>Open your private document library</small></button><button class="native-action-cancel" type="button" onclick="Velora.toggleMobileAttachmentSheet(false)">Cancel</button></div>';
}
function licenseLockPage(){
  var state=String((S.license&&S.license.status)||'inactive').toLowerCase();
  var title=state==='revoked'?'Licence revoked':(state==='expired'?'Licence expired':'Licence required');
  var message=S.licenseLockMessage||'Activate a valid VeloraOS licence to restore access.';
  var admin=!!(S.user&&S.user.role==='admin');
  var actions=admin
    ? '<button class="btn primary" onclick="Velora.go(\'settings\')">Open licence settings</button><button class="btn ghost" onclick="Velora.recheckLicense()">Recheck licence</button>'
    : '<p class="muted">Ask a VeloraOS administrator to renew or activate the licence on this appliance.</p>';
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS entitlement</p><h2>'+escapeHtml(title)+'</h2></div></div>'+
    '<section class="panel licence-lock-panel"><p class="lead">'+escapeHtml(message)+'</p>'+
    '<p>Your local data has not been removed. Chats, settings, Global Memory, Knowledge, installed Ollama models, device identity and TLS/CA material remain on this appliance.</p>'+
    '<div class="license-actions">'+actions+'</div><p class="small muted">Full access returns automatically after a valid licence is activated.</p></section>';
}

function chatHistoryRows(){
  var today=new Date();var startToday=new Date(today.getFullYear(),today.getMonth(),today.getDate()).getTime();var startYesterday=startToday-86400000;
  var groups={today:[],yesterday:[],previous:[]};
  filteredChats().forEach(function(chat){var stamp=Date.parse(chat.updatedAt||chat.createdAt||0)||0;var key=stamp>=startToday?'today':(stamp>=startYesterday?'yesterday':'previous');var active=S.active===chat.id;groups[key].push('<div class="chat-row '+(active?'active':'')+'"><button class="chat-item" '+(active?'aria-current="true" ':'')+'onclick="Velora.openChat(\''+chat.id+'\')" oncontextmenu="event.preventDefault();Velora.openChat(\''+chat.id+'\');Velora.toggleMobileConversationMenu(true)">'+escapeHtml(chat.title||'New chat')+'</button><button class="chat-delete" onclick="Velora.deleteChat(\''+chat.id+'\')" aria-label="Delete '+escapeHtml(chat.title||'chat')+'" title="Delete chat">&times;</button></div>');});
  function section(label,items){return items.length?'<section class="chat-history-section"><h3>'+label+'</h3>'+items.join('')+'</section>':'';}
  return section('Today',groups.today)+section('Yesterday',groups.yesterday)+section('Previous',groups.previous);
}
function chatPage(){
  var editing=S.editingIndex!==null;var promptValue=editing?S.editingDraft:String((S.chatDrafts&&S.chatDrafts[S.active])||'');
  var sidebar='<aside id="chat-history-drawer" class="chat-sidebar'+(S.mobileChatsOpen?' open':'')+'" aria-label="Chat history"><div class="mobile-drawer-head"><div><span class="native-mini-logo">V</span><b id="chat-history-title">VeloraOS</b></div><button id="mobile-drawer-close" type="button" onclick="Velora.toggleMobileChats(false)" aria-label="Close conversations">&times;</button></div><button class="new-chat" onclick="Velora.newChat()">+ New chat</button><label class="sr-only" for="chat-search">Search chats</label><input id="chat-search" class="search" type="search" placeholder="Search conversations" value="'+escapeHtml(S.search)+'" oninput="Velora.setSearch(this.value)"><div class="chat-list" aria-label="Saved chats">'+chatHistoryRows()+'</div><div class="chat-drawer-footer"><button type="button" onclick="Velora.go(\'settings\')">'+avatarHtml(S.user,'drawer-avatar')+'<span><b>'+escapeHtml((S.user&&S.user.display_name)||'User')+'</b><small>Settings</small></span><span aria-hidden="true">›</span></button></div></aside>';
  ...
  var attachInputs='<input id="image-input" type="file" accept="image/*" multiple hidden><input id="camera-input" type="file" accept="image/*" capture="environment" hidden>';
  return '<div class="shell">'+accessibilityChrome()+top()+nativeAppBar()+'<main id="main-content" class="chat-wrap" tabindex="-1">'+sidebar+backdrop+'<section class="chat-main" aria-label="Chat workspace"><div class="chat-top"><div class="chat-title-group"><button id="mobile-chats-button" class="mobile-chats-button" type="button" onclick="Velora.toggleMobileChats(true)" aria-label="Open chat history" aria-controls="chat-history-drawer" aria-expanded="'+(S.mobileChatsOpen?'true':'false')+'">&#9776;</button><h1 class="chat-top-title">'+escapeHtml(currentTitle())+'</h1><button class="chat-title-action" onclick="Velora.renameChat()">Rename</button></div><div class="chat-top-actions"><button id="mobile-chat-plus" class="mobile-chat-plus" type="button" onclick="Velora.toggleMobileChatActions()" aria-haspopup="dialog" aria-expanded="'+(S.mobileChatActionsOpen?'true':'false')+'" title="Chat controls">+</button><button class="desktop-chat-action" onclick="Velora.exportChat(\'markdown\')">Export Markdown</button><button class="desktop-chat-action" onclick="Velora.exportChat(\'json\')">Export JSON</button><button id="lockdown-mode-button" class="desktop-chat-action lockdown-mode-button '+(S.lockdownMode?'active':'')+'" type="button" onclick="Velora.toggleLockdownMode()" aria-pressed="'+(S.lockdownMode?'true':'false')+'" title="Lockdown mode keeps VeloraOS fully offline and blocks all web search and external page fetching.">'+escapeHtml(lockdownButtonLabel())+'</button><button id="coding-mode-button" class="desktop-chat-action coding-mode-button '+(S.codingMode?'active':'')+'" type="button" onclick="Velora.toggleCodingMode()" aria-pressed="'+(S.codingMode?'true':'false')+'" title="Use the strongest installed coding model suitable for this hardware."><span class="coding-mode-label">Coding</span><span class="velora-switch '+(S.codingMode?'on':'')+'" aria-hidden="true"><i></i></span></button></div></div><div class="messages" id="messages" role="log" aria-live="polite" aria-relevant="additions text" aria-busy="'+(S.busy?'true':'false')+'"><div class="messages-inner" id="messages-inner">'+(S.error?'<div class="chat-error" role="alert">'+escapeHtml(S.error)+'</div>':'')+(S.chatNotice?'<div class="chat-notice" role="status">'+escapeHtml(S.chatNotice)+'</div>':'')+(S.msgs.length?S.msgs.map(messageHtml).join(''):emptyChat())+(S.busy?thinkingMessageHtml():'')+'</div></div><div class="composer-wrap"><div class="composer-stack">'+(editing?'<div class="edit-banner" role="status"><div><b>Editing an earlier message</b><span>Sending will replace every response after it.</span></div><button type="button" onclick="Velora.cancelEditMessage()">Cancel</button></div>':'')+'<div class="composer-toolbar"><div class="composer-toolbar-left"><button class="upload-btn" type="button" onclick="Velora.openImagePicker()">Upload image</button><span class="vision-chip">Paste images straight into chat &middot; use Moondream/LLaVA for vision</span></div><div class="composer-toolbar-right">'+cosmicReasoningHtml()+'</div></div>'+pendingImagesHtml()+'<div class="composer"><button class="composer-attach" type="button" onclick="Velora.toggleMobileAttachmentSheet(true)" aria-label="Add image or Knowledge" aria-haspopup="dialog"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg></button><label class="sr-only" for="prompt">Message VeloraOS</label><textarea id="prompt" aria-describedby="composer-hint" placeholder="'+(editing?'Edit this message and resend&hellip;':'Message VeloraOS')+'" rows="1">'+escapeHtml(promptValue)+'</textarea>'+(S.busy?'<button class="send stop" id="send-btn" onclick="Velora.stopGeneration()" aria-label="Stop generation" title="Stop generation"><span aria-hidden="true"></span></button>':'<button class="send" id="send-btn" onclick="Velora.send()" aria-label="Send message"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5M6.5 10.5 12 5l5.5 5.5"/></svg></button>')+'</div><div class="hint" id="composer-hint">Enter to send &middot; Shift+Enter for a new line &middot; copy, edit, regenerate, rename and export are available above</div>'+attachInputs+'</div></div></section></main>'+mobileConversationMenuHtml()+mobileAttachmentSheetHtml()+mobileChatActionsHtml()+mobileBottomNav()+'</div>';
}
...
function syncComposerVisualState(input){
  input=input||$('#prompt');var send=$('#send-btn');var composer=input&&input.closest?input.closest('.composer'):null;var ready=!!((input&&String(input.value||'').trim())||(S.pendingImages&&S.pendingImages.length));
  if(send&&!send.classList.contains('stop')){send.classList.toggle('ready',ready);send.setAttribute('aria-disabled',ready?'false':'true');}
  if(composer)composer.classList.toggle('composer-ready',ready);
}
function afterChat(previousViewport){
  var input=$('#prompt');
  if(input){
    input.addEventListener('input',function(){this.style.height='auto';this.style.height=Math.min(this.scrollHeight,180)+'px';syncComposerVisualState(this);if(S.editingIndex!==null)S.editingDraft=this.value;else{if(!S.chatDrafts)S.chatDrafts={};if(S.active)S.chatDrafts[S.active]=this.value;}});
    input.addEventListener('keydown',function(event){if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();send();}});
    input.addEventListener('paste',handlePaste);
    input.style.height='auto';input.style.height=Math.min(input.scrollHeight,180)+'px';syncComposerVisualState(input);
  ...
  var camera=$('#camera-input');
  if(camera){camera.addEventListener('change',function(event){Velora.handleImageFiles(event.target.files);this.value='';});}
  var box=$('#messages');
  if(box){
...
function modelName(id){return String(id)==='veloraos-coding'?'VeloraOS Coding':'VeloraOS';}
...
function emptyChat(){return '<div class="empty-chat"><div class="empty-orb" aria-hidden="true">'+icons.chat+'</div><h2>How can I help?</h2><p>Private AI running on your VeloraOS.</p></div>';}
...
function stripHiddenReasoningForDisplay(value){
  var text=String(value==null?'':value);
  var tags='think|thinking|analysis|reasoning|reflection|scratchpad';
  var block=new RegExp('<\\s*(?:'+tags+')\\b[^>]*>[\\s\\S]*?<\\s*\\/\\s*(?:'+tags+')\\s*>','gi');
  var previous=null;
  while(previous!==text){previous=text;text=text.replace(block,'');}
  text=text.replace(new RegExp('```(?:'+tags+')\\s*[\\r\\n]+[\\s\\S]*?```','gi'),'');
  var close=new RegExp('<\\s*\\/\\s*(?:'+tags+')\\s*>','i').exec(text);
  if(close)text=text.slice(close.index+close[0].length);
  var open=new RegExp('<\\s*(?:'+tags+')\\b[^>]*>','i').exec(text);
  if(open)text=text.slice(0,open.index);
  text=text.replace(new RegExp('<\\s*\\/?\\s*(?:'+tags+')\\b[^>]*>','gi'),'');
  text=text.trim();
  var lower=text.toLowerCase();
  var prefixes=['okay, the user asked','ok, the user asked','okay, the user is','ok, the user is','okay, the user wants','ok, the user wants','the user asked','the user is','the user wants','let me recall','let me think','let me analyze','let me analyse','first, i need to','first i need to','i need to think','i should think','the user is probably'];
  var looksMeta=prefixes.some(function(prefix){return lower.indexOf(prefix)===0;})||
    ((lower.slice(0,320).indexOf('the user asked')>=0||lower.slice(0,320).indexOf('the user is')>=0||lower.slice(0,320).indexOf('the user wants')>=0)&&(
      lower.slice(0,1200).indexOf('i need to')>=0||
      lower.slice(0,1200).indexOf('let me think')>=0||
      lower.slice(0,1200).indexOf('let me recall')>=0||
      lower.slice(0,1200).indexOf('the user is probably')>=0||
      lower.slice(0,1200).indexOf('previous conversation')>=0
    ));
  if(looksMeta){
    var matches=Array.from(text.matchAll(/(?:^|\n)\s*(?:final answer|answer|response)\s*:\s*/gi));
    if(matches.length){
      var last=matches[matches.length-1];
      text=text.slice(last.index+last[0].length).trim();
    }else{
      return '[Hidden internal reasoning removed]';
    }
  }
  return text;
}
function messageHtml(message,index){var speaker=message.role==='user'?((S.user&&S.user.display_name)||'You'):'VeloraOS';var visibleContent=message.role==='assistant'?stripHiddenReasoningForDisplay(message.content||''):(message.content||'');var body=message.role==='assistant'?renderMarkdown(visibleContent):('<p>'+inlineMarkdown(visibleContent).replace(/\n/g,'<br>')+'</p>');return '<article class="msg" data-message-index="'+index+'" aria-label="Message from '+escapeHtml(speaker)+'">'+chatAvatar(message.role)+'<div class="msg-content"><div class="msg-role">'+escapeHtml(speaker)+'</div><div class="msg-body">'+imageStrip(message.images)+body+'</div>'+messageStatsHtml(message)+messageActionsHtml(message,index)+'</div></article>';}
function setSearch(value){S.search=value;render();}
function toggleMobileChats(open){S.mobileChatsOpen=typeof open==='boolean'?open:!S.mobileChatsOpen;S.mobileConversationOpen=false;S.mobileAttachmentOpen=false;S.menu=false;hapticTick();render();focusAfterRender(S.mobileChatsOpen?'#mobile-drawer-close':'#native-chat-drawer-button');}
function toggleMobileConversationMenu(force){S.mobileConversationOpen=typeof force==='boolean'?force:!S.mobileConversationOpen;S.mobileAttachmentOpen=false;S.mobileChatActionsOpen=false;hapticTick();var sheet=document.querySelector('.native-action-sheet:not(.attachment-sheet)');if(sheet)sheet.classList.toggle('open',S.mobileConversationOpen);var backdrop=document.querySelector('.native-action-backdrop:not(.attachment-backdrop)');if(backdrop)backdrop.hidden=!S.mobileConversationOpen;var button=$('#native-chat-more-button');if(button)button.setAttribute('aria-expanded',S.mobileConversationOpen?'true':'false');}
function toggleMobileAttachmentSheet(force){S.mobileAttachmentOpen=typeof force==='boolean'?force:!S.mobileAttachmentOpen;S.mobileConversationOpen=false;S.mobileChatActionsOpen=false;hapticTick();var sheet=document.querySelector('.attachment-sheet');if(sheet)sheet.classList.toggle('open',S.mobileAttachmentOpen);var backdrop=document.querySelector('.attachment-backdrop');if(backdrop)backdrop.hidden=!S.mobileAttachmentOpen;}
function openMobileChatControls(){toggleMobileConversationMenu(false);toggleMobileChatActions(true);}
function choosePhotoLibrary(){toggleMobileAttachmentSheet(false);setTimeout(openImagePicker,80);}
function chooseCamera(){toggleMobileAttachmentSheet(false);setTimeout(function(){var input=$('#camera-input');if(input)input.click();},80);}
function openKnowledgeFromComposer(){toggleMobileAttachmentSheet(false);go('knowledge');}
async function deleteCurrentChatFromMenu(){var id=S.active;toggleMobileConversationMenu(false);if(id)await deleteChat(id);}
function toggleModelMenu(){S.menu=!S.menu;render();focusAfterRender(S.menu?'.model-option':'#model-picker-button');}
function toggleMobileChatActions(force){
  S.mobileChatActionsOpen=typeof force==='boolean'?force:!S.mobileChatActionsOpen;
  var sheet=$('#mobile-chat-actions');if(sheet)sheet.classList.toggle('open',S.mobileChatActionsOpen);
  var backdrop=$('#mobile-chat-actions-backdrop');if(backdrop)backdrop.hidden=!S.mobileChatActionsOpen;
  var button=$('#mobile-chat-plus');if(button)button.setAttribute('aria-expanded',S.mobileChatActionsOpen?'true':'false');
}
function mobileToggleLockdown(){toggleLockdownMode();toggleMobileChatActions(false);}
function toggleCodingMode(force){
  S.codingMode=typeof force==='boolean'?force:!S.codingMode;
  S.selected=S.codingMode?'veloraos-coding':'veloraos-main';
  var chat=S.chats.find(function(item){return item.id===S.active;});
  if(chat){chat.codingMode=S.codingMode;chat.model=S.selected;chat.updatedAt=new Date().toISOString();saveChats();}
  announce(S.codingMode?'Coding mode enabled.':'Coding mode disabled.');
  render();
}
function previewMobileReasoning(value){var power=clampReasoning(value);var label=$('#mobile-reasoning-value');if(label)label.textContent=String(power);}
function commitMobileReasoning(value){commitCosmicReasoning(clampReasoning(value));}
function selectModel(id){toggleCodingMode(String(id)==='veloraos-coding');}
function newChat(){S.mobileChatActionsOpen=false;S.mobileConversationOpen=false;S.mobileAttachmentOpen=false;S.codingMode=false;S.selected='veloraos-main';var id=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():String(Date.now());S.active=id;S.busy=false;S.chatScrollIntent='bottom';S.msgs=[];S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;S.chats=[{id:id,title:'New chat',model:'veloraos-main',codingMode:false,messages:[],createdAt:new Date().toISOString()}].concat(S.chats);saveChats();render();}
function normaliseChatModelSelection(id){return String(id||'')==='veloraos-coding'?'veloraos-coding':'veloraos-main';}
function chatCodingMode(chat){if(!chat)return false;if(typeof chat.codingMode==='boolean')return chat.codingMode;return normaliseChatModelSelection(chat.model)==='veloraos-coding';}
function openChat(id){S.mobileChatActionsOpen=false;S.mobileConversationOpen=false;S.mobileAttachmentOpen=false;var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;S.chatScrollIntent='bottom';S.active=id;S.msgs=chat.messages||[];S.codingMode=chatCodingMode(chat);S.selected=S.codingMode?'veloraos-coding':'veloraos-main';S.pendingImages=[];S.error='';S.chatNotice='';S.editingIndex=null;S.editingDraft='';S.mobileChatsOpen=false;syncActiveChatBusy();render();}
async function deleteChat(id){
  var chat=S.chats.find(function(item){return item.id===id;});if(!chat)return;
  var confirmed=isStandalonePwa()?await nativeConfirm({title:'Delete chat?',message:'“'+(chat.title||'New chat')+'” will be removed from this device.',confirmLabel:'Delete',destructive:true}):window.confirm('Delete “'+(chat.title||'New chat')+'”? This cannot be undone.');if(!confirmed)return;
  S.chats=S.chats.filter(function(item){return item.id!==id;});
  if(S.active===id){var next=S.chats[0]||null;if(next){S.active=next.id;S.msgs=next.messages||[];S.codingMode=chatCodingMode(next);S.selected=S.codingMode?'veloraos-coding':'veloraos-main';}else{S.active=null;S.msgs=[];}S.pendingImages=[];S.error='';S.editingIndex=null;S.editingDraft='';}
  await saveChats();hapticTick();render();
}
...
function saveActive(){var chat=S.chats.find(function(item){return item.id===S.active;});if(!chat)return saveChats();chat.messages=S.msgs;chat.codingMode=!!S.codingMode;chat.model=S.codingMode?'veloraos-coding':'veloraos-main';chat.updatedAt=new Date().toISOString();if(!chat.createdAt)chat.createdAt=chat.updatedAt;if(!chat.title||chat.title==='New chat'){chat.title=(S.msgs.find(function(message){return message.role==='user';})||{}).content||chat.title||'New chat';if(chat.title.length>42)chat.title=chat.title.slice(0,42);}return saveChats();}
async function renameChat(){var chat=S.chats.find(function(item){return item.id===S.active;});if(!chat)return;var name=isStandalonePwa()?await nativePrompt({title:'Rename chat',message:'Choose a name for this conversation.',value:chat.title||'New chat',confirmLabel:'Save'}):window.prompt('Rename this chat',chat.title||'New chat');if(name===null)return;name=String(name).trim();if(!name)return;chat.title=name.slice(0,80);await saveChats();hapticTick();render();}
...
function exportChat(format){var chat=S.chats.find(function(item){return item.id===S.active;});if(!chat)return;var base=safeFilename(chat.title);if(format==='json'){downloadText(base+'.json',JSON.stringify(chat,null,2),'application/json;charset=utf-8');return;}var lines=['# '+(chat.title||'VeloraOS chat'),'','Mode: '+(chatCodingMode(chat)?'Coding':'VeloraOS Main · Qwen2.5 3B'),''];(chat.messages||[]).forEach(function(message){lines.push('## '+(message.role==='user'?((S.user&&S.user.display_name)||'User'):'VeloraOS'),'',message.role==='assistant'?stripHiddenReasoningForDisplay(message.content||''):(message.content||''));(message.images||[]).forEach(function(image){lines.push('','Attachment: '+((image&&image.name)||'image'));});if(message.stats){lines.push('','_Stats: '+(message.stats.totalTokens||0)+' tokens · '+(message.stats.tokensPerSecond||0)+' tok/s_');}lines.push('');});downloadText(base+'.md',lines.join('\n'),'text/markdown;charset=utf-8');}
...
function copyMessage(index,button){var message=S.msgs[index];if(message)copyPlainText(message.role==='assistant'?stripHiddenReasoningForDisplay(message.content||''):(message.content||''),button);}
...
function beginEditMessage(index){var message=S.msgs[index];if(!message||message.role!=='user'||S.busy)return;S.editingIndex=index;S.editingDraft=message.role==='assistant'?stripHiddenReasoningForDisplay(message.content||''):(message.content||'');S.pendingImages=(message.images||[]).map(function(image){return typeof image==='string'?{dataUrl:image,name:'image'}:{dataUrl:image.dataUrl||image.url||image.data||'',name:image.name||'image'};});S.chatNotice='';render();}
...
async function performGeneration(){
  ...
  try{
    var requestedMode=S.codingMode?'veloraos-coding':'veloraos-main';
    var start=await API.post('/api/chat/tasks',{
      chat_id:S.active,
      model:requestedMode,
      model_id:requestedMode,
    ...
    S.chatTasks[S.active]={id:start.task_id,task_id:start.task_id,kind:'chat',chat_id:S.active,status:start.status||'queued',model:start.model,selected_profile:start.selected_profile,selected_profile_name:start.selected_profile_name,route_reason:start.route_reason,lockdown_mode:!!start.lockdown_mode,web_used:!!start.web_planned,output:start.web_planned?'Preparing web search…':'Responding…'};
    S.chatNotice=(S.codingMode?'Coding · ':'')+(start.selected_profile_name||'VeloraOS')+' · '+(start.route_reason||'Local model')+'. You can leave Chat while it works.';
    ...
    if(error.status===400&&error.data&&error.data.detail&&error.data.detail.code==='invalid_image'){
      S.error=error.data.detail.message||'The image could not be decoded safely. Please upload it again.';
    }
    render();
  }
}
async function send(){
  ...
  var message={id:newMessageId('user'),role:'user',content:effectivePrompt,images:queued,createdAt:new Date().toISOString()};
  ...
  S.pendingImages=[];
  if(S.active)S.chatDrafts[S.active]='';
  if(input){
    input.value='';
...
async function activateLicense(){
  ...
  try{S.license=await API.post('/api/license/activate',{licenseKey:key||null,deviceName:name||null});syncLicenseLockState();setStatus('license-action-status','Licence activated successfully.',false);S.showLicenseKey=false;render();}
  catch(error){setStatus('license-action-status',licenseError(error),true);}finally{key='';S.licenseBusy=false;}
}
async function recheckLicense(){S.licenseBusy=true;setStatus('license-action-status','Checking entitlement…',false);try{S.license=await API.post('/api/license/recheck',{});syncLicenseLockState();setStatus('license-action-status','Entitlement refreshed.',false);render();}catch(error){setStatus('license-action-status',licenseError(error),true);}finally{S.licenseBusy=false;}}
async function deactivateLicense(){if(!confirm('Deactivate this VeloraOS device? It will stop using a device slot, and this installation will require activation again.'))return;S.licenseBusy=true;setStatus('license-action-status','Deactivating device…',false);try{S.license=await API.post('/api/license/deactivate',{});syncLicenseLockState();setStatus('license-action-status','This device has been deactivated.',false);render();}catch(error){setStatus('license-action-status',licenseError(error),true);}finally{S.licenseBusy=false;}}
...
function shortGpuName(gpu){var raw=String((gpu&&gpu.name)||gpu&&gpu.vendor||'CPU only');raw=raw.replace(/^.*?(VGA compatible controller|3D controller|Display controller):\s*/i,'');return raw.length>95?raw.slice(0,92)+'…':raw;}
function systemHealthOverview(){
  var sys=S.system||{};var d=S.diagnostics||{};var o=(d.ollama||((sys.services||{}).ollama)||{});var gpu=sys.gpu||{};var veloraService=((sys.services||{}).veloraos)||{};var healthy=(veloraService.running!==false&&o.running!==false&&o.ready!==false);var os=sys.os||{};
  return '<section class="system-appliance panel"><div class="system-appliance-head"><div><p class="eyebrow">'+escapeHtml(os.name||'VeloraOS 2.0')+'</p><h3>System</h3><p class="muted">Live appliance status and operating-system controls.</p></div><span class="diagnostic-badge '+(healthy?'ready':'not-ready')+'">'+(healthy?'Healthy':'Needs attention')+'</span></div>'+
    '<div class="system-appliance-grid">'+
      diagnosticFact('Status',healthy?'Healthy':'Needs attention')+
      diagnosticFact('Uptime',sys.uptime||'Unknown')+
      diagnosticFact('IP Address',sys.ipAddress||'Not connected',sys.hostname||'')+
      diagnosticFact('CPU',(sys.cpu||{}).model||'Unknown')+
      diagnosticFact('RAM',(sys.memory||{}).total||'Unknown')+
      diagnosticFact('GPU',shortGpuName(gpu),sys.acceleration||'CPU only')+
      diagnosticFact('VeloraOS',sys.version||'1.10.55',os.build?('OS build '+os.build):'Application')+
      diagnosticFact('Ollama',(o.ready||o.running)?'Running':(o.state||o.serviceState||'Needs attention'),o.version||'')+
      diagnosticFact('AI acceleration',sys.acceleration||'CPU only')+
    '</div>'+systemControlPanel()+'</section>';
}
function systemControlPanel(){
  var sys=S.system||{};var available=!!sys.controlsAvailable;var disabled=(S.systemActionBusy||!available)?' disabled':'';var reason=available?'Administrator controls are restricted to four allow-listed system actions.':(sys.controlsReason||'OS controls are not available on this installation.');
  return '<div class="system-control-panel"><div class="system-control-copy"><div><p class="eyebrow">Appliance controls</p><h3>Power & services</h3></div><span class="tag">Administrator only</span></div><p class="muted">'+escapeHtml(reason)+'</p>'+
    '<div class="system-control-actions"><button class="btn ghost"'+disabled+' onclick="Velora.systemAction(\'restart-veloraos\')">Restart VeloraOS</button><button class="btn ghost"'+disabled+' onclick="Velora.systemAction(\'restart-ai\')">Restart AI</button><button class="btn system-danger-soft"'+disabled+' onclick="Velora.systemAction(\'reboot\')">Reboot System</button><button class="danger-button"'+disabled+' onclick="Velora.systemAction(\'shutdown\')">Shut Down</button></div><p id="system-action-status" class="small '+(S.systemActionNotice?'':'muted')+'" role="status" aria-live="polite">'+escapeHtml(S.systemActionNotice||'')+'</p></div>';
}
async function systemAction(action){
  if(S.systemActionBusy)return;var labels={'restart-veloraos':'Restart VeloraOS','restart-ai':'Restart AI','reboot':'Reboot System','shutdown':'Shut Down'};var label=labels[action]||'System action';var warning=action==='shutdown'?'Shut down this VeloraOS appliance now? You will lose access until it is powered on again.':action==='reboot'?'Reboot this VeloraOS appliance now? Active chats and model tasks will be interrupted.':'Run '+label+' now?';var confirmed=await nativeConfirm({title:label,message:warning,confirmLabel:action==='shutdown'?'Shut Down':action==='reboot'?'Reboot':'Restart',destructive:action==='shutdown'||action==='reboot'});if(!confirmed)return;
  S.systemActionBusy=true;S.systemActionNotice=label+' requested…';render();
  try{var result=await API.post('/api/admin/system/'+action,{});S.systemActionNotice=result.message||label+' requested.';nativeToast(S.systemActionNotice,'success');if(action==='restart-veloraos'||action==='reboot'){setTimeout(function(){window.location.href='/app?system_action='+Date.now();},4500);}else if(action==='restart-ai'){setTimeout(async function(){try{S.system=await API.get('/api/system');}catch(_){}S.systemActionBusy=false;render();},2500);}else{S.systemActionBusy=false;render();}}
  catch(error){S.systemActionBusy=false;S.systemActionNotice=humanError(error);render();}
}
...
function systemInfoPage(){
  return '<div class="section-head"><div><p class="eyebrow">VeloraOS administration</p><h2>System</h2><p class="lead">Live hardware, network, AI runtime and appliance controls in one place.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div>'+systemHealthOverview()+secureMobileAccessCard()+
    '<nav class="system-info-jump" aria-label="System sections"><a class="btn ghost" href="#system-diagnostics">Diagnostics</a><a class="btn ghost" href="#system-recovery">Backup & Recovery</a></nav>'+
...
function memorySettingsCard(){
  var memory=S.memory||{items:[],count:0};
  var items=(memory.items||[]).slice().reverse();
  var rows=items.length?items.map(function(item){
    return '<div class="memory-row"><div><b>'+escapeHtml(item.text||'')+'</b><small>'+escapeHtml(item.kind||'memory')+'</small></div><button class="btn ghost compact" onclick="Velora.deleteMemory('+JSON.stringify(String(item.id||''))+')">Forget</button></div>';
  }).join(''):'<p class="muted">Nothing remembered yet. When enabled, VeloraOS can remember useful durable details from normal conversation, such as your name, home area and preferences.</p>';
  return '<div class="setting-card memory-setting-card"><p class="eyebrow">Personal AI</p><h3>Global Memory</h3><div class="setting-toggle-row"><div><b>Remember across chats</b><p class="muted">Private, per-user memory stored locally on this VeloraOS system.</p></div><button class="coding-mode-button '+(S.memoryEnabled?'active':'')+'" type="button" onclick="Velora.toggleMemory()" aria-pressed="'+(S.memoryEnabled?'true':'false')+'"><span class="velora-switch '+(S.memoryEnabled?'on':'')+'" aria-hidden="true"><i></i></span></button></div>'+
    '<p class="small muted">VeloraOS automatically remembers only high-confidence durable details and refuses obvious passwords, tokens, banking identifiers and sensitive health information. Memory is not sent as a database to Web Intelligence.</p>'+
    '<div class="field"><label for="memory-manual">Add a memory</label><div class="memory-add-row"><input id="memory-manual" maxlength="500" placeholder="e.g. I prefer concise answers"><button class="btn ghost compact" onclick="Velora.addMemory()">Remember</button></div></div>'+
    '<div class="memory-list">'+rows+'</div>'+
    (items.length?'<button class="btn ghost compact" onclick="Velora.clearMemory()">Clear all memory</button>':'')+
    '</div>';
}
async function toggleMemory(){
  S.memoryEnabled=!S.memoryEnabled;
  S.settings=Object.assign({},S.settings,{memory_enabled:!!S.memoryEnabled});
  try{
    await API.post('/api/settings',{settings:S.settings});
    if(!S.memory)S.memory={items:[],count:0};
    S.memory.enabled=S.memoryEnabled;
    render();
    announce(S.memoryEnabled?'Global Memory enabled.':'Global Memory disabled. Stored memories are retained until you clear them.');
  }catch(error){
    S.memoryEnabled=!S.memoryEnabled;
    S.settings=Object.assign({},S.settings,{memory_enabled:!!S.memoryEnabled});
    alert(humanError(error));
...
async function addMemory(){
  var input=$('#memory-manual');var text=String(input&&input.value||'').trim();if(!text)return;
  try{await API.post('/api/memory',{text:text,kind:'manual'});S.memory=await API.get('/api/memory');render();announce('Memory saved locally.');}
  catch(error){alert(humanError(error));}
}
async function deleteMemory(id){
  try{await API.del('/api/memory/'+encodeURIComponent(id));S.memory=await API.get('/api/memory');render();announce('Memory forgotten.');}
  catch(error){alert(humanError(error));}
}
async function clearMemory(){
  if(!window.confirm('Clear all Global Memory for this user?'))return;
  try{await API.del('/api/memory');S.memory=await API.get('/api/memory');render();announce('Global Memory cleared.');}
  catch(error){alert(humanError(error));}
}

function settingsPage(){
  if(S.licenseLocked){
    return '<div class="section-head"><div><p class="eyebrow">Licence recovery</p><h2>Restore VeloraOS access</h2><p class="lead">Renew, replace or recheck the appliance licence. Local user data remains preserved.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><div class="settings-grid">'+licensingCard()+profileCard()+accountsCard()+'</div>';
  }
  return '<div class="section-head"><div><p class="eyebrow">Settings</p><h2>Settings</h2><p class="lead">Profiles, memory, separate user accounts, personalisation and system details.</p></div><button class="back" onclick="Velora.go(\'home\')">Back</button></div><div class="settings-grid">'+memorySettingsCard()+pwaSettingsCard()+profileCard()+personalisationCard()+machineCard()+setupSettingsCard()+recoverySettingsCard()+licensingCard()+accountsCard()+'</div>';
}
...
function updateActionsHtml(){
  ...
  if(update.state==='complete'){
    ...
    return '<button class="btn ghost" onclick="Velora.go(\'home\')">Done</button>';
  }
...
async function startAuthenticated(user,csrfToken){
  ...
  try{
    ...
    if(S.setup&&S.setup.required&&!S.licenseLocked){await loadSetupCore();S.setupStep=S.setup.suggestedStep||1;S.page='setup';render();return;}
    if(S.licenseLocked){
      try{var stored=await API.get('/api/chats');S.chats=stored.chats||[];}catch(_){S.chats=[];}
      S.page='home';render();startUpdatePolling();return;
    }
  ...
  else{var chat=S.chats[0];S.active=chat.id;S.msgs=chat.messages||[];S.codingMode=chatCodingMode(chat);S.selected=S.codingMode?'veloraos-coding':'veloraos-main';}
  await loadBackgroundTasks();var launchPage=currentLaunchPage();S.page=launchPage||'home';syncPageUrl(S.page,true);syncActiveChatBusy();render();updateAppBadge();startUpdatePolling();startBackgroundPolling();
}
async function boot(){
...
window.Velora={
  go:go,installModel:installModel,uploadKnowledge:uploadKnowledge,searchKnowledge:searchKnowledge,deleteKnowledge:deleteKnowledge,cancelModelDownload:cancelModelDownload,deleteModel:deleteModel,refreshDiagnostics:refreshDiagnostics,runAccelerationTest:runAccelerationTest,newChat:newChat,openChat:openChat,deleteChat:deleteChat,renameChat:renameChat,exportChat:exportChat,copyMessage:copyMessage,copyCode:copyCode,beginEditMessage:beginEditMessage,cancelEditMessage:cancelEditMessage,regenerateMessage:regenerateMessage,setSearch:setSearch,toggleMobileChats:toggleMobileChats,toggleMobileConversationMenu:toggleMobileConversationMenu,toggleMobileAttachmentSheet:toggleMobileAttachmentSheet,openMobileChatControls:openMobileChatControls,choosePhotoLibrary:choosePhotoLibrary,chooseCamera:chooseCamera,openKnowledgeFromComposer:openKnowledgeFromComposer,deleteCurrentChatFromMenu:deleteCurrentChatFromMenu,toggleMobileChatActions:toggleMobileChatActions,toggleCodingMode:toggleCodingMode,previewMobileReasoning:previewMobileReasoning,commitMobileReasoning:commitMobileReasoning,mobileToggleLockdown:mobileToggleLockdown,toggleModelMenu:toggleModelMenu,selectModel:selectModel,toggleLockdownMode:toggleLockdownMode,send:send,stopGeneration:stopGeneration,toggleCosmicReasoning:toggleCosmicReasoning,previewCosmicReasoning:previewCosmicReasoning,commitCosmicReasoning:commitCosmicReasoning,
...
window.VELORAOS_RELEASE="1.10.55";
