  ...
  try{
    ...
    tasks.forEach(function(task){
      ...
      if(kind==='chat'&&task.chat_id){
        // /api/tasks is newest-first. Keep the first task for each chat so an
        // older completed response cannot overwrite the currently running job.
        if(!nextChats[task.chat_id])nextChats[task.chat_id]=task;
        if(!S.backgroundSeen[id]&&taskIsComplete(task)){
          S.backgroundSeen[id]=true;
          refreshChats=true;
...
function thinkingMessageHtml(){
  return '<article class="msg thinking-message" aria-label="VeloraOS is thinking"><span class="avatar assistant">V</span><div class="msg-content"><div class="msg-role">VeloraOS</div><div class="msg-body"><span class="typing" role="status" aria-label="VeloraOS is thinking"><span></span><span></span><span></span></span></div></div></article>';
}
...
async function performGeneration(){
  ...
  S.busy=true;S.error='';S.chatNotice='VeloraOS is generating this response in the background. You can leave Chat or close the app.';render();window.requestAnimationFrame(scrollChat);
  try{
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.16')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
...
window.VELORAOS_RELEASE="1.10.16";
