...
async function go(page){if(page==='knowledge'){await loadKnowledge();}
...
function mobileChatActionsHtml(){
  ...
  return '<button id="mobile-chat-actions-backdrop" class="mobile-chat-actions-backdrop" type="button" onclick="Velora.toggleMobileChatActions(false)" '+(S.mobileChatActionsOpen?'':'hidden')+' aria-label="Close chat controls"></button>'+
    '<div id="mobile-chat-actions" class="mobile-chat-actions'+(S.mobileChatActionsOpen?' open':'')+'" role="dialog" aria-label="Chat controls">'+
      ...
      '</div>'+
      '<div class="mobile-chat-models"><span class="mobile-chat-actions-label">Model</span>'+models+'</div>'+
    '</div>';
}
function chatPage(){
...
function systemHealthOverview(){var sys=S.system||{};var d=S.diagnostics||{};var o=d.ollama||{};var l=S.license||{};var installed=allModels().filter(function(model){return model.availability&&model.availability.installed;}).length;var gpu=(sys.gpu||{});var health=(o.ready!==false&&l.activated!==false)?'Healthy':'Needs attention';return '<section class="system-overview panel"><div class="system-overview-head"><div><p class="eyebrow">At a glance</p><h3>'+escapeHtml(health)+'</h3></div><span class="diagnostic-badge '+(health==='Healthy'?'ready':'not-ready')+'">'+escapeHtml(health)+'</span></div><div class="system-overview-grid">'+diagnosticFact('VeloraOS',sys.version||'1.10.29')+diagnosticFact('CPU',(sys.cpu||{}).model)+diagnosticFact('Memory',(sys.memory||{}).total)+diagnosticFact('Free disk',(sys.storage||{}).free)+diagnosticFact('GPU',(gpu.name||gpu.vendor||'CPU only'),sys.acceleration||'CPU only')+diagnosticFact('Ollama',o.ready?'Running':'Needs attention',o.version||o.serviceState)+diagnosticFact('Installed models',String(installed))+diagnosticFact('Licence',licenseStateLabel(l.status),l.planName||'')+'</div></section>';}
...
window.VELORAOS_RELEASE="1.10.29";
